...
## Unreleased — Markdown 代码围栏解析修复（issue #9）

- **现象**：用四反引号围栏展示 Markdown 源码的消息被渲染成互不相连的两半——前半显示为可滚动代码卡片，后半（同一围栏内的代码行）却显示为普通段落。
- **根因**：围栏判定是「以三反引号开头」的二元翻转、不记录开启长度；四反引号围栏内的三反引号行被误当作闭合行。
- **修复**：围栏按「开启反引号数量 N」配对——闭合行须仅由 ≥N 个反引号组成（允许尾随空白）；信息串含反引号的行不算围栏；开启行缩进有意不限（保住列表内缩进的代码块）；未闭合仍渲染到结尾。语义对齐 CommonMark：四反引号围栏内全部内容统一渲染为**一个**代码卡片。
- **测试**：新增围栏解析单测（9 个边界）+ 渲染回归（5 个，含旧实现下失败的区分断言）。
...
- 表格单元格按行内规则渲染（`**加粗**`、`` `代码` ``、链接），不再显示字面 `**` 与反引号。
- 修复列宽测量未计入环境文本样式与系统字号、导致单元格意外换行的问题；超宽表格保留横向滚动。
- 新增 App 回归测试 `md_table_test.dart`（同列跨行对齐 / 行内标记 / 横向滚动）。仅影响表格渲染，未改动其它块级与行内逻辑。
...
- 悬浮球面板新增「用量与额度」区块（ADR 0008）：展开时按需获取 + 客户端节流（2 分钟），不可用（旧插件 / 未配置 / 失败）时整体降级为原有单行余额；金额行保留文字（CNY 优先）、配额行只出细条与颜色、不出数字（每个进度条上方居中显示 5h / 每周 / 每月 窗口短标签）；面板不显示账户身份与附加 Codex bucket；配额窗口永不相加、不参与预警。点击区块/「详情 ▸」直达 App 用量页，「去充值」保留。顺带修复悬浮球余额取数非 CNY 优先（与详情页不一致）的问题。新增纯 Kotlin 面板模型 seam（JVM 单测 27 例）。
...
- **仍未完成的环境矩阵**：2026-09-17 已用本机 Android SDK/JDK 17 成功构建 Release APK（`3.1.3+21`，74,303,262 bytes，签名证书 SHA-256 `fe1f2af1…209c`）；当前仍无 Android 设备，因此 T02/T19 真机交互无法执行。没有 DSH Desktop 宿主实例，`0.1.5-rc.2 × Desktop` 组合未执行；无低于下界的独立宿主，T20 低版本安装实测仅由自动化替身覆盖。真实宿主正在承载本会话，未做会中插件卸载/热重载；T11 真实用户凭据未被改动（保存/清除由隔离契约测试覆盖）；T15 当前部署 `pushChannels=0`，真实 ntfy wire 由隔离测试覆盖；T16 无活跃后台任务/子代理可供取消或中断。
