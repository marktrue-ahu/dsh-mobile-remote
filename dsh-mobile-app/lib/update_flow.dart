...
Future<void> runUpdateFlow(
  ...
  if (result == 'cancelled') {
    ...
    showToast(
      ...
      L10n.t(
        '已拉起安装器，请按系统提示完成安装',
        'Installer opened — follow the system prompt',
      ),
    );
  } else {
          ...
          if (c.notes != null && c.notes!.isNotEmpty) ...[
            ...
            ConstrainedBox(
              ...
              child: SingleChildScrollView(
                child: Text(
                  c.notes!,
                  style: TextStyle(fontSize: 12.5, color: DshColors.ink2(ctx)),
                ),
              ),
            ),
          ],
...
class _DownloadDialogState extends State<_DownloadDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      child: AlertDialog(
        title: Text(
          _error != null || _done ? L10n.t('更新', 'Update') : _stageLabel,
        ),
        content: Builder(
          builder: (ctx) {
            if (_error != null) {
            ...
            if (_done && _result == '') {
              return Text(
                L10n.t(
                  '已拉起安装器，返回本页后继续使用 App',
                  'Installer opened — return to keep using the app',
                ),
                style: TextStyle(color: ink3),
              );
            }
