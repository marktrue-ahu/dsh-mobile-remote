...
class Updater {
  static const _githubRepo =
      'https://api.github.com/repos/201222-L/dsh-mobile-remote/releases/latest';
  ...
  static Future<UpdateCheckOutcome> check(AppStore store) async {
    ...
    if (local == null) {
      return const UpdateCheckOutcome(
        verdict: UpdateVerdict.upToDate,
        error: '无法读取本地版本',
      );
    }
    ...
    try {
      ...
      return UpdateCheckOutcome(
        verdict: UpdateVerdict.upToDate,
        error: _checkErrorMessage(store.updateSource, e),
      );
    }
    if (candidate == null) {
    ...
    if (remote == null) {
      return UpdateCheckOutcome(
        verdict: UpdateVerdict.upToDate,
        error: '更新源版本号格式异常：${candidate.version}',
      );
    }
    return UpdateCheckOutcome(
  ...
  static Future<UpdateCandidate?> _candidateFromHost() async {
    ...
    return candidateFromHostManifest(
      d,
      baseUrl: hostApiBase(baseUrl: api.baseUrl, mountPath: api.path),
    );
  }
  ...
  static Future<UpdateCandidate?> _candidateFromGithub() async {
    ...
    final resp = await http
        .get(
          Uri.parse(_githubRepo),
          headers: {'User-Agent': 'dsh-mobile-remote-app'},
        )
  ...
  static Future<String?> downloadAndInstall(
    ...
    final target = File(
      '${updateDir.path}/dsh-update-${DateTime.now().millisecondsSinceEpoch}.apk',
    );
    ...
    try {
      ...
      if (candidate.sha256 != null && candidate.sha256!.isNotEmpty) {
        ...
        await sha256.bind(target.openRead()).fold<AccumulatorSink<Digest>>(
          acc,
          (a, d) {
            a.add(d);
            return a;
