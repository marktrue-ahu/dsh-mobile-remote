...
export function createGitService(ctx, { onChanged } = {}) {
	...
	const run = async (argv, cwd, signal) => {
	...
	const context = async ({ sessionId, cwd } = {}) => {
		...
		return { repositoryId: repositoryId(root), root, name: root.split(/[\\/]/).at(-1) || root, capabilities: capabilities() };
	};
	...
	const capabilities = () => { const provider = Boolean(subprocessOf()?.spawn); const workspace = registryAvailable(); return { available: provider && workspace, read: provider && workspace, writes: false, reason: !provider ? "git-provider-unavailable" : !workspace ? "workspace-registry-unavailable" : null, features: { status: true, branches: true, graph: true, commit: true, diff: true, branchSwitch: false, createBranch: false, fetch: false, pull: false, push: false, tags: false } }; };
	const rootFor = async (repositoryIdValue) => {
		const root = await assertAllowed(repositoryIdValue);
		return gitRoot(root);
	};
	...
	const service = {
		capabilities,
		context,
		async status(id) { const root = await rootFor(id); observedRoots.add(root); return { repositoryId: root, ...parseGitStatus(await run(["-C", root, "status", "--porcelain=v1", "-z", "-b"], root)) }; },
		async branches(id) { const root = await rootFor(id); const text = await run(["-C", root, "for-each-ref", "--format=%(refname)%09%(objectname)%09%(upstream:short)%00", "refs/heads", "refs/remotes"], root); return { repositoryId: root, branches: parseGitBranches(text) }; },
		async graph(id, { limit = 100, cursor, refs } = {}) {
			...
			if (snapshot.tips.length === 0) return { repositoryId: root, commits: [], snapshotId: snapshot.id, tips: [], nextCursor: null };
