...
export function createGitService(ctx, { onChanged } = {}) {
	...
	const context = async ({ sessionId, cwd } = {}) => {
		...
		return { repositoryId: repositoryId(root), name: root.split(/[\\/]/).at(-1) || "repository", capabilities: capabilities() };
	};
