	...
	const handleApi = async (req, res, url, rest) => {
		...
		if (rest === "/bootstrap") {
			...
			sendJson(res, 200, {
				...
				git: { ...git.capabilities(), operations: gitOperations.capabilities?.() ?? null },
				agents: agentList,
		...
		if (rest === "/git/capabilities") {
			if (requireGet(method, res)) return;
			sendJson(res, 200, { ok: true, git: { ...git.capabilities(), operations: gitOperations.capabilities?.() ?? null } });
			return;
		}
