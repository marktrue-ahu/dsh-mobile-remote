...
export function apply(ctx, config) {
	...
	const promptImage = (sessionId, mode, content) => apiRpc("session.prompt", { sessionId, mode, content, requestId: randomUUID() }, 120_000);
	...
	const RPC_PAYLOAD_ADAPTER = {
		// ADR 0001：requestId 是内核 strict schema 的**必填**字段（SessionRequestId），
		// 缺失会被网关以 gateway/input-invalid 直接拒绝投递——不是「仅失去幂等保护」。
		// 故在此无条件铸造，不依赖调用方传入。该 id 同时是内核 hasPromptRequest 幂等层的依据；
		// 插件自建回执层（/send-receipt）与之职责不同并各自保留：内核负责不重复执行同一请求，
		// 插件负责传输层中断后回答「是否已送达」。
		"session.prompt": (p) => ({ request: { requestId: p.requestId ?? randomUUID(), sessionId: p.sessionId, mode: p.mode, content: p.content } }),
		...
		"subagent.interrupt": (p) => ({ childSessionId: p.childSessionId, parentSessionId: p.parentSessionId, mode: p.mode }),
		"goal.create": (p) => ({ agent: p.sessionId, request: { objective: p.objective, ...(p.maxGoalRounds === undefined ? {} : { maxGoalRounds: p.maxGoalRounds }) } }),
	};
	...
	const handleApi = async (req, res, url, rest) => {
		...
		if (rest === "/respond") {
			...
			if (body?.kind === "question") {
				...
				const qEntry = pendingQuestions.get(qPhoneId)
					?? [...pendingQuestions.values()].find((e) => e.rpcId === rpcId);
				if (qEntry && Array.isArray(body?.answers)) {
				...
				const aEntry = pendingApprovals.get(aPhoneId)
					?? [...pendingApprovals.values()].find((e) => e.rpcId === rpcId);
				if (aEntry) {
					// outcome: "allowed-once" | "rejected" | 其他 → fail-close（unavailable）
					finishApproval(aEntry, outcome === "allowed-once" || outcome === "rejected" ? outcome : "unavailable");
					sendJson(res, 200, { ok: true, accepted: true });
					return;
				}
			} else if (body?.kind === "cancel") {
				// v3.1.3：现代宿主（无 apiProxy）下取消 = 结算本地条目——审批 → cancelled、
				// 问询 → 跳过（null，与超时语义一致）；v3.1.2 只能走 apiProxy 通道，
				// 现代内核下取消悬空、条目只能等 120s 超时。未命中（对端已先答）继续走降级。
				...
				if (cancelEntry) {
					...
					else finishQuestion(cancelEntry, null);
					sendJson(res, 200, { ok: true, accepted: true });
					return;
				}
			}
	...
	const finishQuestion = (entry, value) => {
		if (!pendingQuestions.has(entry.phoneId)) return false;
		pendingQuestions.delete(entry.phoneId);
		if (entry.timer) { clearTimeout(entry.timer); entry.timer = null; }
		ctx.logger.info?.(`mobile-remote: user-questions/request 结算（${entry.phoneId}）`);
	...
	// 超时 fail-close。rpcId 置空串——App 端帧 rpcId 与条目结算按 phoneId 匹配（历史契约）。
	const holdApproval = (req, sessionId) => {
		...
		return new Promise((resolve) => {
			const entry = { phoneId: phoneKey, sessionId, rpcId: "", timer: null, settle: resolve };
