...
export function apply(ctx, config) {
	...
	/** 日志脱敏：抹掉绝对路径等本机信息（宿主错误 message 可能带工作区路径；日志若外送
	 *  会泄露布局，HTTP 响应只用固定文案，此处只影响服务端/推送日志）。 */
	const REDACT_PATH_PATTERN = /(?:\/[A-Za-z0-9_.-]+){2,}|[A-Za-z]:\\[^\s"']*/;
	const failureDetail = (err) => String(err?.message ?? String(err ?? "unknown")).replace(REDACT_PATH_PATTERN, "<redacted>");
