...
import { chmodSync, closeSync, existsSync, fsyncSync, mkdirSync, openSync, readFileSync, writeSync, renameSync, writeFileSync } from "node:fs";
...
export function apply(ctx, config) {
	...
	/** 消息内容经内核 session.prompt 发送（与 PC 端同 wire；RC1 负责恢复冷会话）。 */
	const promptContent = (sessionId, mode, content, requestId = randomUUID()) => apiRpc(
		"session.prompt",
		{
			...(hasRc1Gateway() ? { requestId } : {}),
			sessionId,
			mode,
			content,
		},
	...
	const promptImage = promptContent;
	/** RC1 冷会话提示：优先使用已注入的 Host service，避免 Gateway 查找层失配。 */
	const promptRc1 = (sessionId, mode, content, requestId) => {
		const request = { requestId, sessionId, mode, content };
		if (remoteSessionController && typeof remoteSessionController.prompt === "function") {
			return remoteSessionController.prompt(request, AbortSignal.timeout(120_000));
		}
		return promptContent(sessionId, mode, content, requestId);
	};
	...
	const releaseHeld = (sessionId) => {
		...
		for (const h of held) {
			if (h.images?.length) {
				promptImage(agent.id, "queue", heldContentOf(h), h.id).catch((err) => {
	...
	// DSH 0.1.2-rc.1：Typert Gateway 与旧 apiProxy 并存时，所有 Host 调用与
	// 问询/审批帧都优先走 Gateway；旧协议仅在 Gateway 未注入时使用。
	let remoteGateway = null;
	let remoteSessionController = null;
	let remoteFrameAbort = null;
	let remoteEventClientId = null;
	const remotePendingFrames = new Map(); // mobile rpcId -> { clientId, eventId, kind, sessionId }
	const hasRc1Gateway = () => !!(remoteGateway && typeof remoteGateway.invoke === "function");
	const hasRc1EventBridge = () => !!(
		hasRc1Gateway()
		&& remoteFrameAbort
		&& !remoteFrameAbort.signal.aborted
		&& typeof remoteEventClientId === "string"
		&& remoteEventClientId !== ""
	);
	...
	/** RC1 Remote Event waterfall → 旧版移动端 question/approval 帧。 */
	const publishRemoteWaterfall = (frame) => {
		if (!frame || frame.type !== "waterfall" || typeof frame.event !== "string" || typeof frame.eventId !== "string") return;
		...
		if (frame.event === "user-questions/request") {
			...
			const existing = remotePendingFrames.get(rpcId);
			if (existing) {
				// Gateway 会在 Remote Event stream 重连时重放仍未结算的事件；
				// 移动端已有 pending 帧，更新 clientId 即可，避免重复弹窗。
				existing.clientId = remoteEventClientId;
				return;
			}
			const pending = { clientId: remoteEventClientId, eventId: frame.eventId, kind: "question", sessionId };
			remotePendingFrames.set(rpcId, pending);
			const mobileFrame = {
			...
			pendingFrameSet(`q:${rpcId}`, mobileFrame);
			broadcast({ type: "mobile/frame", frame: mobileFrame });
			if (sessionId) {
				notifyNeedsAnswer(sessionId, "agent 正在等待你的回答");
			}
			return;
		}
		if (frame.event === "approval/request") {
			// RC1 approval/request 的请求体没有旧 API 的 approvalId；Gateway
			// eventId 是同一请求的稳定相关标识，作为移动端 approvalId 使用。
			const rpcId = frame.eventId;
			const existing = remotePendingFrames.get(rpcId);
			if (existing) {
				existing.clientId = remoteEventClientId;
			...
			const pending = { clientId: remoteEventClientId, eventId: frame.eventId, kind: "approval", sessionId };
			remotePendingFrames.set(rpcId, pending);
			const mobileFrame = {
				...
				toolName: typeof request.toolName === "string" ? request.toolName : "工具",
				...(typeof request.callId === "string" ? { callId: request.callId } : {}),
				...(typeof request.reason === "string" ? { reason: request.reason } : {}),
			};
			pendingFrameSet(`a:${frame.eventId}`, mobileFrame);
			broadcast({ type: "mobile/frame", frame: mobileFrame });
			if (sessionId) {
				notifyNeedsAnswer(sessionId, `审批请求：${mobileFrame.toolName} 需要授权`);
			}
		}
	};
	const completeRemoteWaterfall = (rpcId, pending, { cancelled = false } = {}) => {
		remotePendingFrames.delete(rpcId);
		if (pending.kind === "question") {
			pendingFrames.delete(`q:${rpcId}`);
			...
			pendingFrames.delete(`a:${pending.eventId}`);
			broadcast({ type: "mobile/frame", frame: {
	...
	/** Send a mobile answer through RC1's special `$events/result` endpoint. */
	const respondRemoteWaterfall = async (body) => {
		const rpcId = String(body?.rpcId ?? "");
		const pending = remotePendingFrames.get(rpcId);
		if (!pending) return false;
		if (body?.kind !== "cancel" && pending.sessionId && String(body?.sessionId ?? "") !== pending.sessionId) {
			throw Object.assign(new Error("sessionId does not match the pending interaction"), { status: 400, code: "session-mismatch" });
		}
		if (typeof pending.clientId !== "string" || pending.clientId === "") {
			throw Object.assign(new Error("RC1 Remote Event stream is reconnecting"), { status: 503, code: "respond-unavailable" });
		}
		const clientId = pending.clientId;
		let outcome;
		if (body?.kind === "question") {
			if (pending.kind !== "question") throw Object.assign(new Error("pending interaction is not a question"), { status: 400, code: "interaction-kind-mismatch" });
			if (!Array.isArray(body?.answers)) throw Object.assign(new Error("question respond expects answers[]"), { status: 400, code: "bad-request" });
			outcome = { kind: "result", value: { answers: body.answers } };
		} else if (body?.kind === "approval") {
			if (pending.kind !== "approval") throw Object.assign(new Error("pending interaction is not an approval"), { status: 400, code: "interaction-kind-mismatch" });
			if (body?.approvalId !== undefined && String(body.approvalId) !== pending.eventId) {
				throw Object.assign(new Error("approvalId does not match the pending interaction"), { status: 400, code: "approval-mismatch" });
			}
			const approvalOutcome = String(body.outcome ?? "");
			if (!["allowed-once", "rejected", "cancelled", "unavailable"].includes(approvalOutcome)) {
				throw Object.assign(new Error("approval outcome is invalid"), { status: 400, code: "approval-outcome-invalid" });
			}
			outcome = { kind: "result", value: approvalOutcome };
		} else if (body?.kind === "cancel") {
			// Question cancellation must reject the waterfall so the user-question
			// service settles with ASK_ABORTED. Approval cancellation is a valid
			// closed outcome and therefore remains fail-closed but non-throwing.
			outcome = pending.kind === "question"
				? { kind: "rejected", error: { name: "UserQuestionError", message: "mobile interaction cancelled", code: "ASK_ABORTED", details: {} } }
				: { kind: "result", value: "cancelled" };
		} else {
			throw Object.assign(new Error("kind must be question | approval | cancel"), { status: 400, code: "bad-request" });
		}
		const result = {
			clientId,
		...
		// RC1：`$events/result` 只能经 Typert Gateway 的进程内 `dispatchRpc`
		// 结算（其 /api HTTP 面被浏览器 session 认证栅挡住，旧版 apiRpcTransport
		// 的 client-request envelope 会 401）。dispatchRpc 不抛错，失败回
		// { ok:false, error } —— 需显式转成 rpcError 可透传的错误。
		const settle = await remoteGateway.dispatchRpc(RC1_REMOTE_EVENT_RESULT, { args: result }, AbortSignal.timeout(30_000));
		if (settle?.ok !== true) {
		...
		// A reconnect can replace the Gateway client while the result request is
		// in flight. The Gateway treats a result for the retired client as an
		// idempotent no-op; keep the local pending frame in that race so the
		// re-delivered event remains answerable instead of disappearing.
		if (remotePendingFrames.get(rpcId) !== pending) return true;
		if (pending.clientId !== clientId) {
			throw Object.assign(new Error("RC1 Remote Event stream reconnected before the answer was accepted"), {
				status: 503,
				code: "respond-retry",
			});
		}
		completeRemoteWaterfall(rpcId, pending, { cancelled: body?.kind === "cancel" });
		return true;
	};
	const consumeRemoteFrame = (frame) => {
		if (!frame || typeof frame.type !== "string") return;
		if (frame.type === "ready") {
			remoteEventClientId = typeof frame.clientId === "string" ? frame.clientId : null;
			for (const pending of remotePendingFrames.values()) pending.clientId = remoteEventClientId;
			return;
		}
		if (frame.type === "waterfall") {
			publishRemoteWaterfall(frame);
			return;
		}
		if (frame.type === "cancel" && typeof frame.eventId === "string") {
			for (const [rpcId, pending] of remotePendingFrames) {
				if (pending.eventId !== frame.eventId) continue;
				completeRemoteWaterfall(rpcId, pending, { cancelled: true });
				break;
			}
		}
		// RC1 emit frames are already represented by ctx.on("session/event") /
		// ctx.on("agent/status") in this plugin; forwarding them again would duplicate SSE.
	};
	const startRemoteEventBridge = (gateway) => {
		if (remoteFrameAbort) {
			remoteFrameAbort.abort();
			remoteFrameAbort = null;
		}
		remoteEventClientId = null;
		for (const pending of remotePendingFrames.values()) pending.clientId = null;
		if (!gateway?.wireStream?.open) return;
		const controller = new AbortController();
		remoteFrameAbort = controller;
		(async () => {
			let announcedFailure = false;
			while (!controller.signal.aborted) {
				try {
					const stream = await gateway.wireStream.open("$events", { args: {} }, controller.signal);
					announcedFailure = false;
					for await (const frame of stream) {
						if (controller.signal.aborted) return;
						consumeRemoteFrame(frame);
					}
					if (controller.signal.aborted) return;
					throw new Error("RC1 Remote Event stream ended unexpectedly");
				} catch (err) {
					if (controller.signal.aborted) return;
					remoteEventClientId = null;
					for (const pending of remotePendingFrames.values()) pending.clientId = null;
					if (!announcedFailure) {
						ctx.logger.warn(`mobile-remote: RC1 Remote Event bridge waiting: ${err?.message ?? err}`);
						announcedFailure = true;
					}
					await new Promise((resolve) => setTimeout(resolve, 1000));
				}
			}
		})().catch((err) => ctx.logger.warn(`mobile-remote: RC1 Remote Event bridge stopped: ${err?.message ?? err}`));
	};
	...
	// ── 移动端 v2 公共 helper ─────────────────────────────────────
	/** 通过 /api 桥发送一个 Connection RPC envelope（旧 API Proxy 与 RC1 共用载体）。 */
	const apiRpcTransport = async (method, payload, timeoutMs = 15000) => {
	...
	/** 通过 /api 桥调用旧版 API Proxy（与浏览器 GUI 同一 HTTP 协议）。 */
	const legacyApiRpc = apiRpcTransport;
	/**
	 * 当前 Host Remote 调用的唯一适配入口。
	 * RC1 直接调用 in-process Typert Gateway，旧版保留 API Proxy envelope；业务错误
	 * 不在两条协议之间重试，避免一次用户操作被重复执行。
	 */
	const apiRpc = async (method, payload, timeoutMs = 15000) => {
		if (hasRc1Gateway()) {
			const request = rc1RemoteSpec(method, payload);
			try {
				const value = await remoteGateway.invoke({
					...request,
					signal: AbortSignal.timeout(timeoutMs),
				});
				return method === "session.models" ? normalizeRc1ModelCatalog(value) : value;
			} catch (err) {
				// Gateway 的 RemoteError 已携带稳定 code；保留它，只补充移动 API 所需
				// 的 status 字段，不回退到旧端点。
				const e = err instanceof Error ? err : new Error(String(err));
				if (e.code === undefined && typeof err?.code === "string") e.code = err.code;
				if (e.status === undefined) e.status = e.code === "gateway/cancelled" ? 504 : 400;
				throw e;
			}
		}
		return legacyApiRpc(method, payload, timeoutMs);
	};
	...
	const readSessionConfig = async (sessionId) => {
		...
		try {
			let current;
			if (hasRc1Gateway()) {
				current = foldRc1ModelSelection(events);
				if (!current) {
					const list = await apiRpc("session.list", {}, 15000);
					current = modelSelectionFromRc1SessionList(list, sessionId);
				}
				// A brand-new RC1 Session has no selection projection yet; use the
				// deployment default so the existing create-without-model flow keeps
				// its explicit model binding behavior.
				if (!current) current = (await apiRpc("session.models", {}, 15000))?.current;
			} else {
				current = (await apiRpc("session.models", { sessionId }))?.current;
			}
		...
		if (session) {
			const permissionPresets = ctx.get("permissionPresets");
			try {
				config.permissionPreset = permissionPresets?.current(events);
			} catch {
				// 保持 undefined
			}
			config.agentPreset = foldAgentPreset(events);
		}
		return config;
	};
	...
	const handleApi = async (req, res, url, rest) => {
		...
		if (rest === "/send") {
			...
			const sendRoute = rc1SendRoute({
				gatewayAvailable: hasRc1Gateway(),
				sessionControllerAvailable: !!(remoteSessionController && typeof remoteSessionController.prompt === "function"),
				sessionId,
				liveAgent: target,
			});
			if (sendRoute.kind === "missing") return error(res, sessionId ? 404 : 503, sessionId ? "session-not-found" : "no-live-agent");
			const targetId = target?.id ?? sendRoute.sessionId;
			const receiptKey = requestId ? receiptKeyOf(sessionId, targetId, requestId) : null;
			if (receiptKey) {
			...
			ctx.logger.info?.(`mobile-remote: /send hit → ${targetId} text=${text.length}B imgs=${images.length} steer=${steer} status=${target?.status ?? "cold"}`);
			const message = createUserMessage({ content: [{ type: "text", text }], source: { kind: "user" } });
			if (sendRoute.kind === "resume") {
				// RC1 的 session.prompt 会先 resolve/resume 持久化会话，再把内容交给 Agent。
				// 这条路径覆盖手机从会话列表打开的 dormant Session；旧版 DSH 仍由上面的
				// live-agent 分支处理，避免改变旧版排队/插队语义。
				try {
					await promptRc1(sendRoute.sessionId, steer ? "steer" : "queue", content, requestId ?? message.id);
					finalize(res, 200, {
						ok: true,
						accepted: true,
						agentId: sendRoute.sessionId,
						mode: steer ? "steer" : "followup",
						note: "session-resumed",
					});
				} catch (err) {
					const notFound = err?.code === "session/not-found";
					finalize(res, notFound ? 404 : 500, {
						error: notFound ? "session-not-found" : "send-failed",
						detail: err?.message ?? String(err),
					});
				}
				return;
			}
			const prompt = (mode) => promptImage(target.id, mode, content, message.id);
				...
				try {
					if (action.kind === "edit") {
						...
						if (item.images?.length) {
							...
							await promptImage(sessionId, running ? "steer" : "queue", heldContentOf(item), item.id);
						} else {
		...
		if (rest === "/history") {
			...
			let sessionEvents = sessionEventsOf(session);
			...
			if (!session || sessionEvents.length === 0) {
				...
				if (query?.readSession) {
					try {
						...
						if (!session) session = { events: snapshotEvents };
						if (sessionEvents.length === 0) sessionEvents = snapshotEvents;
					} catch {
			...
			const surface = sessionEvents.filter((event) => SURFACE_TYPES.has(event.type));
			let events;
			if (afterParam !== null) {
		...
		if (rest === "/catalog") {
			...
			const imageLimitsOf = async (sessionId) => {
				...
				try {
					let limits;
					if (hasRc1Gateway()) {
						// RC1 图片限制位于 session.list 行的 projections.values.imageLimits。
						const list = await apiRpc("session.list", {}, 15000);
						limits = imageLimitsFromRc1SessionList(list, sessionId);
					} else {
						const value = await apiRpc("session.history", { sessionId, maxMessages: 1 }, 15000);
						limits = value?.projections?.attachments?.imageLimits ?? null;
					}
		...
		if (rest === "/respond") {
			// 移动端回答内核问询/审批：RC1 通过 `$events/result` 结算
			// Remote Event waterfall；旧版继续经 apiProxy.respond。
			if (requirePost(method, res)) return;
			const rc1 = hasRc1Gateway();
			if (rc1) {
				if (!hasRc1EventBridge()) {
					return error(res, 503, "respond-unavailable", "RC1 Remote Event 通道不可用（请检查 dsh Gateway）");
				}
			} else if (!proxy || typeof proxy.respond !== "function") {
				return error(res, 503, "respond-unavailable", "内核问询/审批通道不可用（请升级 dsh）");
			}
			const body = await readJson(req, res);
			if (body === undefined) return;
			const rpcId = String(body?.rpcId ?? "");
			...
			if (rc1) {
				if (!remotePendingFrames.has(rpcId)) return error(res, 404, "respond-not-found", "未找到待处理的问询或审批");
				try {
					await respondRemoteWaterfall(body);
					sendJson(res, 200, { ok: true, accepted: true });
				} catch (e) {
					ctx.logger.warn(`mobile-remote: RC1 respond failed: ${e?.message ?? e}`);
					return error(res, ...rpcError(e, "respond-failed"));
				}
				return;
			}
			if (!proxy || typeof proxy.respond !== "function") return error(res, 404, "respond-not-found", "未找到待处理的问询或审批");
			let message;
			if (body?.kind === "question") {
		...
		if (rest === "/usage") {
			...
			const session = sessions?.get(sessionId);
			if (!session) return error(res, 404, "session-not-found");
			const sessionEvents = sessionEventsOf(session);
		...
		if (rest === "/diagnostics") {
			...
			const services = {
				...
				gitService: true,
				typertGateway: hasRc1Gateway(),
				sessionController: !!(remoteSessionController && typeof remoteSessionController.prompt === "function"),
			};
			...
			checks.respondBridge = hasRc1Gateway()
				? hasRc1EventBridge()
				: !!(proxy && typeof proxy.respond === "function");
			checks.frameBridge = hasRc1Gateway()
				? hasRc1EventBridge()
				: !!(frameAbort && !frameAbort.signal.aborted);
...
git: git.diagnostics(),
				protocol: hasRc1Gateway() ? "typert-rc1" : "api-proxy-legacy",
				remoteEventClientId,
				services,
				checks,
				notes: [
	...
	ctx.effect(() => {
		...
		try {
			ctx.inject(["apiProxy"], (apiCtx) => {
				...
				if (hasRc1Gateway()) {
					// 混合宿主同时暴露旧 apiProxy 与 RC1 Gateway 时，避免两条
					// 事件流把同一个问询/审批推送两次。
					frameAbort = null;
					return;
				}
		...
		try {
			ctx.inject(["sessionController"], (apiCtx) => {
				const service = apiCtx.sessionController;
				remoteSessionController = service;
				return () => {
					if (remoteSessionController === service) remoteSessionController = null;
				};
			});
		} catch {
			// 旧版 DSH 或精简 profile 没有 SessionController，继续使用 Gateway/旧协议。
		}
		// DSH 0.1.2-rc.1：Gateway 是新 Remote 调用与 Remote Event 的统一入口。
		// ctx.inject 负责跨插件可见性；未挂载 RC1 Gateway 时保持旧 apiProxy 路径。
		try {
			ctx.inject(["typertGateway"], (apiCtx) => {
				remoteGateway = apiCtx.typertGateway;
				if (frameAbort) {
					frameAbort.abort();
					frameAbort = null;
				}
				startRemoteEventBridge(remoteGateway);
			});
		} catch {
			// 0.1.1-rc.2 没有 Typert Gateway，继续使用旧协议。
		}
		// ── LAN 桥（v2.9.0）━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
		...
		return () => {
			...
			if (remoteFrameAbort) remoteFrameAbort.abort();
			proxy = null;
			frameAbort = null;
			remoteFrameAbort = null;
			remoteGateway = null;
