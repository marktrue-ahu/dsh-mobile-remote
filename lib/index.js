	...
	const handleApi = async (req, res, url, rest) => {
			...
			try {
				...
				try {
					if (typeof body.model === "string") {
						...
						// v3.1.0 修复：无显式 model/reasoningEffort 时，跟随内核默认模型并绑定——
						// 否则首次运行提示词组装 {{model}} 无值，会话直接不可用（复现：POST /m/api/sessions 不带 model）
						const config = await readSessionConfig(sessionId);
						if (typeof config.model === "string" && config.model !== "") {
							try {
								...
								// 绑定失败：回滚会话（不留不可用会话），明确报错
								await apiRpc("session.cancel", { sessionId }).catch(() => {});
			...
			try {
				...
				const friendly =
					/(^|\s)(401|403)\b|answered 401|answered 403|HTTP 401|HTTP 403/.test(raw)
						...
						: /could not reach|ECONNREFUSED|ETIMEDOUT|timeout/i.test(raw)
		...
		if (rest === "/diagnostics") {
			...
			sendJson(res, 200, {
				...
				checks,
				notes: [
					// v3.1.0：内核侧已知限制披露——dsh-tool-skill 的目录发布条件（agent 工具视图能
					// 解析到 skill 工具）未覆盖插件 agents.create 创建的会话：移动端新建会话不注入
					// 技能目录（PC 端 GUI 会话正常；用户点名技能仍可注入）。详见 docs/09 §2.3 与
					// 上游 deepseek-harness issue（2026-08）。
					"skill-catalog: 移动端新建会话不注入技能目录（DSH 内核限制）；用户点名技能仍可调用，详见 docs/09-compatibility.md",
				],
			});
			return;
		}
