...
import { chmodSync, closeSync, createReadStream, existsSync, fsyncSync, mkdirSync, openSync, readFileSync, statSync, realpathSync, writeSync, renameSync, writeFileSync } from "node:fs";
...
export function apply(ctx, config) {
	...
	const existingWorkspaceFile = (candidate) => {
		try {
			const target = realpathSync(resolve(normalizeServerPath(candidate)));
			return pathWithinWorkspaceRoots(target, workspaceRootRealpaths()) && statSync(target).isFile() ? target : null;
		} catch { return null; }
	};
	const existingWorkspaceDirectory = (candidate) => {
		try {
			const target = realpathSync(resolve(normalizeServerPath(candidate)));
			return pathWithinWorkspaceRoots(target, workspaceRootRealpaths()) && statSync(target).isDirectory() ? target : null;
		} catch { return null; }
	};
	const handleApi = async (req, res, url, rest) => {
		...
		if (rest === "/respond") {
			...
			if (body?.kind === "question") {
				const qPhoneId = String(body?.questionId ?? "");
				...
				if (qEntry) {
					if (String(body?.sessionId ?? "") !== qEntry.sessionId) return error(res, 400, "session-mismatch");
				...
				const aPhoneId = String(body?.approvalId ?? "");
				...
				if (aEntry) {
					if (String(body?.sessionId ?? "") !== aEntry.sessionId) return error(res, 400, "session-mismatch");
					if (aPhoneId && aPhoneId !== aEntry.phoneId) return error(res, 400, "approval-mismatch");
					const acceptedOutcome = outcome === "allowed-once" || outcome === "rejected" ? outcome : "unavailable";
					if (!await finishApproval(aEntry, acceptedOutcome)) return error(res, 503, "respond-unavailable", "审批结算失败，请重试");
					sendJson(res, 200, { ok: true, accepted: true });
					return;
				}
				...
				if (cancelEntry) {
					if (body?.sessionId && String(body.sessionId) !== cancelEntry.sessionId) return error(res, 400, "session-mismatch");
					const accepted = pendingApprovals.has(cancelEntry.phoneId)
						? await finishApproval(cancelEntry, "cancelled")
						: await finishQuestion(cancelEntry, null);
		...
		if (rest === "/files" && method === "GET") {
			...
			const target = existingWorkspaceFile(requestedPath);
			...
			try {
				...
				return error(res, 404, "file-not-found", err?.message ?? String(err));
			}
			return;
		}
		if (rest === "/files/upload" && method === "POST") {
			...
			const safeDir = typeof dir === "string" ? existingWorkspaceDirectory(dir) : null;
			if (!safeDir) return error(res, 404, "workspace-not-found", "无法确定授权工作区目录");
			const target = join(safeDir, name);
			if (existsSync(target)) return error(res, 409, "file-exists", "目标文件已存在");
			try {
	...
	const finishApproval = async (entry, outcome) => {
		if (!pendingApprovals.has(entry.phoneId) || entry.settling) return false;
		entry.settling = true;
		try {
	...
	const finishQuestion = async (entry, value) => {
		if (!pendingQuestions.has(entry.phoneId) || entry.settling) return false;
		entry.settling = true;
		try {
			const result = await entry.settle(value);
			if (result === false) {
				entry.settling = false;
				return false;
			}
	...
	const holdApproval = (req, sessionId) => {
		...
		return new Promise((resolve) => {
			const entry = { phoneId: phoneKey, sessionId, rpcId: phoneKey, timer: null, settling: false, settle: (outcome) => { resolve(outcome); return true; } };
	...
	const holdQuestion = (req, sessionId) => {
		...
		return new Promise((resolve) => {
			...
			armTimeout(entry, () => {
				ctx.logger.warn?.(`mobile-remote: user-questions/request 手机超时未应答 → 跳过（${phoneKey}）`);
				finishQuestion(entry, null);
			});
			broadcast({ type: "mobile/frame", frame: {
	...
	// $events 事件处理：瀑布经内核转发到达网关后才投给本客户端（手机在场才建条目/弹卡；
	// 无手机时桌面 GUI 独自呈现，本客户端只收 cancel 帧清理——无条目则自然无操作）。
	const onEventsWaterfall = (frame) => {
		...
		if (!sessionId || !eventId || settledEventIds.has(eventId)) return;
		...
		if (frame.event === "approval/request") {
			...
			const entry = {
				phoneId: phoneKey, sessionId, rpcId: eventId, timer: null, settling: false,
				settle: (outcome) => postEventsValue(eventId, outcome),
			};
			pendingApprovals.set(phoneKey, entry);
			armTimeout(entry, () => {
		...
		if (frame.event === "user-questions/request") {
			...
			const entry = {
				...
				timer: null,
				settling: false,
				settle: (value) => postEventsValue(eventId, value),
			};
			pendingQuestions.set(phoneKey, entry);
			armTimeout(entry, () => {
	...
	ctx.effect(() => {
		...
		return () => {
			...
			for (const entry of pendingQuestions.values()) {
				if (entry.timer) clearTimeout(entry.timer);
				try { Promise.resolve(entry.settle(null)).catch(() => {}); } catch {}
			}
