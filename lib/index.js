 ...
 * [1, 2) 内，且同次版本时 patch 不低于下界。即「当前 Typert 代际及其后续补丁/RC」——
 * 换代际需要重新适配，因此更高的版本（0.2.x / 1.x）一律不放行。
...
export function apply(ctx, config) {
	...
	const hostVersion = () => {
		...
		const candidates = ["@deepseek-ai/dsh", "@deepseek-ai/dsh-api-gateway", "@deepseek-ai/dsh-host-webserver"];
		let req;
		try {
