...
export const isTimelineRecord = (event) => Boolean(
	event && typeof event.type === "string" && !REPLAY_IGNORED_TYPES.has(event.type)
);
	...
	switch (type) {
		...
		case "tool/result": {
			...
			const images = Array.isArray(data.images)
				...
				: [];
			const files = [
				...filesOfNested(data.files, true),
				...filesOfNested(data.file, true),
				...filesOfNested(blocks),
			];
			...
			return {
				...
				data: {
					...
					step: data.step,
					callId,
					name: directName || errName || callId || "tool",
					isError,
					text: clampText(text, 2000),
					...(images.length ? { images: images.slice(0, 20) } : {}),
					...(files.length ? { files: files.slice(0, 20) } : {}),
				},
			};
		}
		case "todo/write": {
...
export function summarizeEvent(event) {
	...
	// 拒绝（isTimelineRecord），声明 available 只会让 App 显示一个必然 404 的按钮。
	if (!isTimelineRecord(event)) return summary;
	return {
		...summary,
		detail: { available: true, seq: event.seq },
	};
}
...
git: { ...gitCapabilities(), operations: gitOperations.capabilities?.() ?? null },
		...
		if (rest === "/event-detail") {
			...
			if (query?.readEvent) {
				try {
					...
					if (candidate?.type && Number(candidate.seq) === seqNumber && candidateIdentityMatches && sourceIdentityMatches && isTimelineRecord(candidate)) event = candidate;
				} catch {
					// 旧 query 或未注册该方法时回退到快照。
				}
			}
			...
			if (event && !isTimelineRecord(event)) event = null;
		...
		if (rest === "/history") {
			...
			// 只过滤明确的内部/敏感记录；其它未知事件保留，避免新事件静默丢失。
	...
	// v3.1.4（issue #14 Bug1+Bug2）：**不再因手机离线而早退**。
	// 无论手机是否在线都建条目 + 写回放存储；只在手机在线时 fail-close，
	// 离线条目走 armOfflineReap。保留 RC1 agentId 兼容解析。
	const onEventsWaterfall = (frame) => {
		...
		if (frame.event === "user-questions/request") {
			...
			if (phoneOnline) {
				armTimeout(entry, () => {
					ctx.logger.warn?.(`mobile-remote: user-questions/request 双端待办超时 → 跳过（${phoneKey}）`);
					void finishQuestion(entry, null);
				});
			} else {
