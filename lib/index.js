...
export function apply(ctx, config) {
	...
	const pruneReceipts = () => {
		const now = Date.now();
		for (const [k, v] of sendReceipts) {
			if (now - v.at > RECEIPT_TTL) sendReceipts.delete(k);
		}
		while (sendReceipts.size > RECEIPT_MAX) {
			sendReceipts.delete(sendReceipts.keys().next().value);
		}
	};
	const loadReceipts = () => {
		try {
	...
	const handleApi = async (req, res, url, rest) => {
		...
		if (rest === "/send") {
			...
			if (receiptKey) {
				const existing = sendReceipts.get(receiptKey);
				if (existing) {
		...
		if (rest === "/send-receipt") {
			...
			const entry = sendReceipts.get(key);
