...
import { chmodSync, closeSync, createReadStream, existsSync, fsyncSync, mkdirSync, openSync, readFileSync, statSync, realpathSync, writeSync, renameSync, writeFileSync, constants as fsConstants } from "node:fs";
...
import { createGitOperationManager, GitOperationError } from "./git-operations.js";
import { acceptedOperationResponse, createGitWriteService } from "./git-write-service.js";
import { createGitRemoteService } from "./git-remote-service.js";
import {
	TYPERT_REMOTE_EVENT_RESULT,
	...
	sessionEventsOf,
} from "./typert-adapter.js";

/** Cordis 插件名（cordis.patch.yml 中按此 id 引用）。 */
export const name = "mobile-remote";

const PROVIDER_SECRET_BYTES = 32;
const providerSecretPath = () => join(homedir(), ".dsh", "mobile-remote", "provider-secret");
/** Load the provider HMAC key without ever putting it in DTOs or logs. */
function loadProviderSecret() {
	const directory = join(homedir(), ".dsh", "mobile-remote");
	mkdirSync(directory, { recursive: true, mode: 0o700 });
	try { chmodSync(directory, 0o700); } catch {}
	const path = providerSecretPath();
	try {
		const secret = readFileSync(path);
		if (secret.length !== PROVIDER_SECRET_BYTES) throw new Error("invalid provider secret length");
		try { chmodSync(path, 0o600); } catch {}
		return secret;
	} catch (error) {
		if (error?.code !== "ENOENT") throw error;
	}
	const secret = randomBytes(PROVIDER_SECRET_BYTES);
	try {
		const fd = openSync(path, "wx", 0o600);
		try { writeSync(fd, secret); fsyncSync(fd); } finally { closeSync(fd); }
		try { chmodSync(path, 0o600); } catch {}
		return secret;
	} catch (error) {
		if (error?.code !== "EEXIST") throw error;
		const existing = readFileSync(path);
		if (existing.length !== PROVIDER_SECRET_BYTES) throw new Error("invalid provider secret length");
		return existing;
	}
}
/**
...
export const Config = z.object({
	...
	/**
	 * App 自动更新「主机源」目录（v3.x）：里面放发布产出的 APK 与 manifest.json
	 * （由发布脚本生成，manifest 为唯一权威）。空 = 未配置（App 查询时返回明确错误）。
	 * 支持 `~/` 前缀（展开到 homedir）。经 /m/api/update/* 端点 + authToken 提供更新。
	 */
...
/** 受支持宿主的最小版本（与 package.json peerDependencies 下界同源，ADR 0001：Issue #9）。 */
export const SUPPORTED_HOST_MIN = [0, 1, 5];
/** 受支持宿主的最大次版本（不含）——同主版本内高于此值即超出代际范围。 */
export const SUPPORTED_HOST_MAX_MINOR = 2;
/** 受支持宿主范围的可读形态（诊断展示；与 package.json 的 peerDependencies 保持一致）。 */
export const SUPPORTED_HOST_RANGE = `>=0.1.5-rc.2 <0.2.0`;

/**
 * 宿主版本是否落在受支持范围内（顶层纯函数，便于单测）。
 *
 * 语义对齐 peer 范围 `>=0.1.5-rc.2 <0.2.0`：主版本必须相同，次版本落在
 * [1, 2) 内，且同次版本时（patch、prerelease）不低于下界。即「当前 Typert
 * 代际及其后续补丁/RC」——换代际需要重新适配，因此更高的版本（0.2.x / 1.x）
 * 一律不放行；**低于下界的同 tuple prerelease（如 0.1.5-rc.1）同样不放行**。
 * 传入无法解析的值返回 null（未知），由调用方决定如何表述，不谎报为不支持。
 */
export function isHostVersionSupported(version) {
	if (typeof version !== "string") return null;
	const m = /^(\d+)\.(\d+)\.(\d+)(?:-([0-9A-Za-z.-]+))?(?:\+[0-9A-Za-z.-]+)?$/.exec(version.trim());
	if (!m) return null;
	const [major, minor, patch] = [Number(m[1]), Number(m[2]), Number(m[3])];
	const prerelease = m[4] === undefined ? null : m[4].split(".");
	const [minMajor, minMinor, minPatch] = SUPPORTED_HOST_MIN;
	// 主版本必须一致：换代际（无论更高还是更低）都不在受支持范围内。
	if (major !== minMajor) return false;
	// 次版本区间 [minMinor, MAX_MINOR)：上界为开区间，0.2.x 已属下一代际。
	if (minor < minMinor || minor >= SUPPORTED_HOST_MAX_MINOR) return false;
	// 次版本更高则必然满足下界（允许同代后续补丁/RC）。
	if (minor > minMinor) return true;
	// 同次版本：patch 低于下界 → 不支持；patch 高于下界 → 支持；
	// patch 相等时按 semver 比较 prerelease：无 prerelease（正式版）高于一切 pre，
	// 有 prerelease 则必须 ≥ 下界的 prerelease（如 rc.2；rc.1 / alpha 均低于下界）。
	if (patch > minPatch) return true;
	if (patch < minPatch) return false;
	const minPre = "rc.2".split("."); // 与 SUPPORTED_HOST_RANGE 下界同源
	if (prerelease === null) return true;
	// semver prerelease 比较：数值段按数值、字符串段按字典序、公共段相等时短者小。
	for (let i = 0; i < Math.max(prerelease.length, minPre.length); i += 1) {
		const a = prerelease[i];
		const b = minPre[i];
		if (a === undefined) return false; // 短者小：rc < rc.2
		if (b === undefined) return true;  // 长者大：rc.2 > rc
		if (a === b) continue;
		const na = /^\d+$/.test(a) ? Number(a) : null;
		const nb = /^\d+$/.test(b) ? Number(b) : null;
		if (na !== null && nb !== null) return na > nb;
		if (na !== null) return false; // 数值段 < 字符串段（semver 规则）
...
/**
 * 解析 App 更新目录配置：空串 = 未配置；`~/` 前缀展开到 homedir。
 * 返回绝对路径；未配置返回 ""。
 */
...
/** 读取 updateDir 下的 manifest.json；返回 { ok, error?, manifest? }（错误码供端点映射）。 */
function readUpdateManifest(dir) {
	...
	try {
		...
		return { ok: false, error: "update-manifest-invalid" };
	}
	if (
		!raw ||
		typeof raw !== "object" ||
	...
	if (apk === "" || !existsSync(apk) || !statSync(apk).isFile()) {
		return { ok: false, error: "update-apk-missing" };
	}
	return {
		ok: true,
		manifest: {
			version: raw.version,
			apk: raw.apk,
			...(Number.isFinite(raw.size) && raw.size > 0 ? { size: raw.size } : {}),
...
/** 校验 APK 文件名不会逃逸 updateDir（防路径遍历；路径由部署者 manifest 控制，双保险）。 */
function apkPathInDir(dir, name) {
	const target = join(dir, name);
	const resolved = target.startsWith(dir.endsWith("/") ? dir : `${dir}/`);
	if (!resolved || name.includes("..") || name.includes("/") || name.includes("\\")) return "";
	return target;
}
...
export function apply(ctx, config) {
	...
	const promptImage = (sessionId, mode, content, requestId = randomUUID()) =>
		apiRpc("session.prompt", { requestId, sessionId, mode, content }, 120_000);
	...
	// ── 宿主代际与能力探测（ADR 0001：Issue #9）──────────────────────────
	// 兼容性判定按**能力**而不是版本号：版本号只用于诊断展示与升级提示，不参与功能门控。
	// 拆成三个互相独立的能力项，因为「能调用」与「能收事件」是独立成立的两件事——
	// 旧一代宿主恰好存在「有 Remote 网关服务与 invokeRpc 方法、却没有事件流面」的组合，
	// 把两者合成一个布尔值会得到「声称新代而事件桥静默不启动」的不一致结论。
	/** 宿主 DSH 版本号（诊断展示用；读不到返回 null，不参与门控）。 */
	const hostVersion = () => {
		// 官方同款 idiom：沿 node 解析链找清单文件（不能探目录存在性——profile 的
		// 依赖镜像只增不删，升级后会留下指向已不存在包的断链，探目录会误判为存在）。
		// 优先读按 profile bundles 安装的**内核包**（dsh-llm 等与宿主整体同代同版），
		// 再退到 CLI 根包 @deepseek-ai/dsh——全局 CLI 可能与 profile 宿主版本脱节
		// （如 CLI 0.1.5-rc.1 + profile 0.1.5-rc.2），诊断应反映实际运行的内核。
		const candidates = ["@deepseek-ai/dsh-llm", "@deepseek-ai/dsh-api-gateway", "@deepseek-ai/dsh", "@deepseek-ai/dsh-host-webserver"];
		let req;
		try {
		...
		for (const pkg of candidates) {
			try {
				for (const searchPath of req.resolve.paths(pkg) ?? []) {
					const manifest = join(searchPath, pkg, "package.json");
	...
	/** 能力探测：三项独立判定，任一缺失使对应功能以明确错误失败（不静默降级）。 */
	const hostCapabilities = () => {
		...
		const gateway = service("typertGateway");
		return {
			/** 新代 Remote 调用入口：模型目录、会话配置、发送等全部依赖它。 */
			hasRemoteInvoke: !!(gateway && typeof (gateway.invokeRpc ?? gateway.invoke) === "function"),
			/** Remote 事件桥入口：问询/审批双端呈现（$events）的前提。 */
			hasRemoteEventBridge: !!(gateway && typeof gateway.openWireStream === "function"),
			/** 冷会话恢复入口：不在 live agent 注册表里的持久会话发送。 */
			hasColdSessionResume: typeof service("sessionController")?.prompt === "function",
			/** 旧代交互通道（旧代宿主专用；存在即说明宿主未升级，见上方弃用日志）。 */
			hasLegacyInteractionBridge: typeof service("apiProxy")?.respond === "function",
		};
	};
	/**
	 * 受支持宿主范围的可读形态来自顶层常量 `SUPPORTED_HOST_RANGE`（与 package.json 的
	 * peerDependencies 同源），仅用于诊断判定，不参与门控。
	 *
	 * 注意范围形态：`^0.1.5-rc.2` 这种 caret 写法在 npm 语义下**不**匹配 `0.1.5-rc.3`
	 * （prerelease 只有与下界完全同 tuple 才被允许），与「支持当前代际含后续 RC」的意图不符，
	 * 故显式写成「≥ 下界且同主版本」的区间。
	 */

	/** 宿主节（诊断用）：版本号 + 能力 + 是否受支持。版本号读不到时 supported 记 null（未知）。 */
	const hostReport = () => {
		const version = hostVersion();
		return {
			version,
			supported: isHostVersionSupported(version),
			supportedRange: SUPPORTED_HOST_RANGE,
			capabilities: hostCapabilities(),
		};
	};
	/** 升级提示文案（诊断 notes 与能力缺失错误共用）。 */
	const HOST_UPGRADE_HINT = `请把电脑端 DSH 升级到 ${SUPPORTED_HOST_RANGE} 或同代更高版本`;
	/** 宿主节的一句话结论（诊断 notes；详尽结果在 host 节本身）。 */
	const hostNote = () => {
		const report = hostReport();
		const capabilities = report.capabilities;
		const missing = [
			capabilities.hasRemoteInvoke ? null : "Remote 调用入口",
			capabilities.hasRemoteEventBridge ? null : "Remote 事件桥",
			capabilities.hasColdSessionResume ? null : "冷会话恢复",
		].filter(Boolean);
		const version = report.version === null ? "版本未知" : `DSH ${report.version}`;
		if (missing.length > 0) {
			return `host: ${version}；能力缺失：${missing.join("、")} → 对应功能会以明确错误失败，${HOST_UPGRADE_HINT}`;
		}
		if (report.supported === false) {
			return `host: ${version} 在受支持范围（${SUPPORTED_HOST_RANGE}）之外，但三项能力均就绪；${HOST_UPGRADE_HINT}`;
		}
		if (report.supported === true) return `host: ${version}（受支持范围 ${SUPPORTED_HOST_RANGE}），三项能力均就绪`;
		return `host: ${version}；三项能力均就绪（版本号无法解析，未做范围判定）`;
	};
	...
	/**
	 * RFC 2047 encoded-word：把含非 ASCII 的文本编码成 ASCII 安全的头值。
	 *
	 * 必要性：Node 的 undici 会对非 Latin-1 的头值直接抛 TypeError
	 * （Cannot convert argument to a ByteString…value…greater than 255），
	 * 而推送标题含中文与 emoji（如「🔔 测试通知」）。
	 *
	 * 纯 ASCII 原样返回；否则按 UTF-8 码点分片编码——每片独立成合法 UTF-8
	 * （不可按字节切，否则会劈开多字节字符），45 字节 → base64 60 字符，
	 * 加 `=?UTF-8?B?` + `?=` 包装仍 < 75（RFC 2047 上限）。多片以空格连接，
	 * 解码方按 RFC 2047 丢弃连接空白并拼接。ntfy 实测能正确解码回原文。
	 */
	const encodeRfc2047 = (text) => {
		...
		const flush = () => {
			if (buf === "") return;
			words.push(`=?UTF-8?B?${Buffer.from(buf, "utf8").toString("base64")}?=`);
			buf = "";
		};
		for (const ch of value) {
			if (Buffer.byteLength(buf + ch, "utf8") > 45) flush();
			buf += ch;
		}
		flush();
		return words.join(" ");
	...
	const pushToChannel = async (target, kind, kindLabel, title, detail, sessionId) => {
		...
		if (target.format === "serverchan") {
			...
			// ntfy：配置里的 URL 是 topic URL（https://ntfy.sh/<topic>），此形态**必须**用
			// text/plain 正文 + X-Title 头承载标题。
			//   · JSON 发布只对根 URL 生效（body 需带 topic）；发到 topic URL 会被 ntfy
			//     当作纯文本消息 → 标题丢失、正文变成裸 JSON（Issue #12）。
			//   · 「别用 X-Title」的顾虑并非空穴来风——但限制不在 ntfy，而在 Node 的 undici：
			//     非 Latin-1 头值会抛 TypeError，而标题含中文/emoji。解法不是改投 JSON，
			//     而是按 RFC 2047 编码——头值保持 ASCII 安全，ntfy 展示前解码回原文
			//     （实测中文与 emoji 完整还原）。
			init = { method: "POST", headers: { ...headers, "content-type": "text/plain; charset=utf-8", "x-title": encodeRfc2047(`${kindLabel} · ${title}`) }, body: desp };
	...
	const error = (res, status, err, detail, metadata = {}) => sendJson(res, status, { error: err, ...(detail ? { detail } : {}), ...metadata });
	const requireGitContract = (req, res) => {
		const raw = req.headers["x-dsh-git-contract"] ?? req.headers["x-dsh-git-mobile-contract"];
		if (typeof raw !== "string" || raw.trim() === "") return error(res, 409, "client-incompatible", "a compatible Git mobile contract version is required", { requiresRefresh: true });
		const match = /^(\d+)\.(\d+)(?:\.(\d+))?$/.exec(raw.trim());
		if (!match || Number(match[1]) !== 2 || Number(match[2]) < 0) return error(res, 409, "client-incompatible", "Git mobile contract version is incompatible", { requiresRefresh: true });
		return false;
	};
	const gitActor = () => config.authToken ? `auth:${createHash("sha256").update(config.authToken).digest("hex")}` : "auth-disabled";
	const rejectUnknownKeys = (body, allowed, res) => {
		const unknown = Object.keys(body).filter((key) => !allowed.has(key));
		return unknown.length > 0 ? error(res, 400, "invalid-argument", `unsupported request field: ${unknown[0]}`) : false;
	};
	...
	// Slice B0 operation seam: the task manager is deliberately independent of
	// Git command execution. Future Git write adapters register executors here;
	// B0 only exposes durable task/query/cancel/recovery semantics.
	let gitOperations;
	let ownsGitOperations = false;
	try { gitOperations = ctx.get("gitOperations"); } catch { gitOperations = undefined; }
	if (!gitOperations) {
		ownsGitOperations = true;
		gitOperations = createGitOperationManager({
			filePath: join(homedir(), ".dsh", "mobile-remote", "git-operations.log"),
		});
		ctx.provide("gitOperations", gitOperations);
		void gitOperations.start().catch((err) => ctx.logger.warn?.(`mobile-remote: Git operation provider unavailable: ${err?.code ?? err?.message ?? err}`));
	}
	const unsubscribeGitOperations = gitOperations.onEvent?.((frame) => broadcast(frame));
	const providerSecret = loadProviderSecret();
	let gitWrites;
	let ownsGitWrites = false;
	try { gitWrites = ctx.get("gitWriteService"); } catch { gitWrites = undefined; }
	if (!gitWrites) {
		ownsGitWrites = true;
		gitWrites = createGitWriteService({ git, operations: gitOperations, preconditionSecret: providerSecret, onChanged: (repositoryId, changeKinds) => broadcast({ type: "git/changed", repositoryId, changeKinds }) });
		ctx.provide("gitWriteService", gitWrites);
	}
	let gitRemote;
	let ownsGitRemote = false;
	try { gitRemote = ctx.get("gitRemoteService"); } catch { gitRemote = undefined; }
	if (!gitRemote) {
		ownsGitRemote = true;
		gitRemote = createGitRemoteService({ git, operations: gitOperations, preconditionSecret: providerSecret, onChanged: (repositoryId, changeKinds) => broadcast({ type: "git/changed", repositoryId, changeKinds }) });
		ctx.provide("gitRemoteService", gitRemote);
	}
	const gitCapabilities = () => {
		const base = git.capabilities();
		const writes = gitWrites.capabilities?.() ?? {};
		const remoteWrites = gitRemote.capabilities?.() ?? {};
		const features = { ...(base.features ?? {}), ...(writes.features ?? {}), ...(remoteWrites.features ?? {}) };
		// Providers historically used both createBranch/branchCreate spellings. Keep
		// aliases coherent so capability negotiation never exposes contradictory flags.
		for (const [canonical, alias] of [["createBranch", "branchCreate"], ["branchSwitch", "switchBranch"], ["branchRename", "renameBranch"]]) {
			if (features[canonical] === true || features[alias] === true) features[canonical] = features[alias] = true;
		}
		return {
			...base,
			...writes,
			...remoteWrites,
			// `available` describes the read provider, not each optional write
			// adapter. A missing remote seam must not hide B1/B2 or read Git.
			available: base.available,
			read: base.read,
			writes: writes.writes === true || remoteWrites.writes === true,
			reason: base.reason ?? writes.reason ?? remoteWrites.reason ?? null,
			features,
		};
	};
	...
	let proxy = null; // ctx.get("apiProxy")（含 respond / events.mux）；仅旧代宿主提供（ADR 0001 弃用通道）
	let legacyHostWarned = false; // 旧代宿主弃用提示只打一次（apiProxy 命中即旧代）
	...
	const pendingFrameDelete = (kind, rpcId) => {
		if (typeof rpcId !== "string" || rpcId === "") return;
		pendingFrames.delete(`${kind}:${rpcId}`);
	};
	...
	const onSessionEvent = (session, event) => {
		...
		// 队列即时同步（ADR 0001，T08/R5）：内核任何生效的 inbox 变更都会 append
		// `agent/inbox/spliced` 会话事件，经本监听器到达——据此重推 mobile/queue 快照，
		// 让电脑端改动与内核认领即时反映到手机 dock，不再依赖轮询兜底。
		// 该事件在上一代与当前代载荷一致（{target,start,removedCount?,inserted,outcome?}，
		// 无 sessionId 字段），会话身份取自本监听器的第一个参数。
		if (event.type === "agent/inbox/spliced") {
			broadcastQueue(session.id);
		}
	...
	// ── 移动端 v2 公共 helper ─────────────────────────────────────
	// 内核 RPC 契约（当前代际）：
	// ① 端点命名 namespace/method（session.models → session/modelCatalog、goal.create → goals/create、
	//    subagent.* → subagents.*）；
	// ② 载荷按新参数形态适配（单 request 包装 / 多参数平铺 / 零参数，见网关 typert 定义）；
	// ③ 桌面 2.0.5 起 /api HTTP 通道被浏览器访问门禁+会话 Cookie 鉴权关闭（插件 fetch 必 403），

	//    统一走进程内 ctx.typertGateway（@Remote 网关，与宿主同域直通服务层）。
	// 旧宿主（无网关）保留原 HTTP /api 降级路径——该路径依赖旧代 apiProxy 的 /api envelope，
	// 属 ADR 0001 的弃用通道：仍可用但不受支持，命中时打一次 warn（见下方帧桥）。
	/** 能力缺失时的统一错误（ADR 0001：明确报错，不静默降级、不伪装成功）。 */
	const capabilityError = (capability, detail) =>
		Object.assign(new Error(`${detail}（宿主能力缺失：${capability}）`), {
			status: 503,
			code: "host-capability-unavailable",
		});
	//    RC1/RPC endpoint/argument mapping is centralized in typert-adapter.js.  Do not
	//    derive plural namespaces or agent-scoped arguments from legacy method names.
	...
	const apiRpc = async (method, payload, timeoutMs = 15000) => {
		...
		if (gateway && typeof gateway.invokeRpc === "function") {
			...
			const full = await gateway.invokeRpc(endpoint, { args: request.args });
			if (full?.ok === true) return method === "session.models" ? normalizeRc1ModelCatalog(full.value) : full.value;
			const e = new Error(full?.error?.message ?? `${method} failed`);
			e.status = 400;
			e.code = full?.error?.code;
			throw e;
		}
		// 旧代降级路径（ADR 0001 弃用通道）：仅在宿主既无新代调用入口、又确有旧代
		// apiProxy 服务时才尝试；两者皆无 → 明确报错，不把「试一个必然 403 的死端点」
		// 伪装成内核故障（那正是本 ADR 要消除的静默失效）。
		const legacyProxy = (() => {
			try {
		...
		if (typeof legacyProxy?.respond !== "function") {
			if (!legacyHostWarned) {
				legacyHostWarned = true;
				ctx.logger.warn?.(`mobile-remote: 宿主既无 Remote 调用入口（typertGateway.invokeRpc）也无旧代 apiProxy —— ${method} 无法执行；${HOST_UPGRADE_HINT}`);
			}
			throw capabilityError("hasRemoteInvoke", `${method} 无法执行：宿主未提供 Remote 调用入口。${HOST_UPGRADE_HINT}`);
		}
	...
	const readSessionConfig = async (sessionId) => {
		...
		try {
			...
			const selection = projectionOf(control, sessionId)?.modelSelection;
			const current = selection?.next ?? selection?.lastUsed ?? foldRc1ModelSelection(sessionEventsOf(session));
			config.model = current?.model;
			config.provider = current?.provider;
			config.reasoningEffort = current?.reasoningEffort;
		} catch (err) {
		...
		if (session) {
			...
			try {
				...
				if (query?.readSession) {
					...
					Object.assign(config, foldFromEvents(snapshot?.events ?? []));
				}
			} catch {
				// 保持 undefined
			}
	...
	const workspaceRootRealpaths = () => {
		const roots = ctx.get("workspaceRegistry")?.list?.() ?? [];
		return roots.flatMap((workspace) => {
			if (typeof workspace?.path !== "string" || workspace.path === "") return [];
			try { return [realpathSync(resolve(normalizeServerPath(workspace.path)))]; } catch { return []; }
		});
	};
	const pathWithinWorkspaceRoots = (target, roots) => roots.some((root) => pathWithinWorkspace(root, target));
	/**
	 * Return a stable path rooted at an already-open directory/file descriptor.
	 * Linux and macOS expose descriptors through procfs/devfs; using that path
	 * for the upload child prevents a checked parent directory from being
	 * swapped for a symlink before the child is opened.
	 */
	const descriptorFsAvailable = (() => {
		const root = process.platform === "linux" ? "/proc/self/fd" : process.platform === "darwin" ? "/dev/fd" : "";
		if (!root) return false;
		try { return existsSync(root) && realpathSync(root) !== ""; } catch { return false; }
	})();
	const descriptorPath = (handle, suffix = "") => {
		if (!descriptorFsAvailable || !Number.isInteger(handle?.fd)) return null;
		if (process.platform === "linux") return `/proc/self/fd/${handle.fd}${suffix}`;
		return `/dev/fd/${handle.fd}${suffix}`;
	};
	const actualPathOfHandle = (handle) => {
		const descriptor = descriptorPath(handle);
		if (!descriptor) return null;
		try {
			// Do not trust the pathname used to open the object after the open:
			// fds remain attached to the object even if its directory is renamed.
			return realpathSync(descriptor);
		} catch { return null; }
	};
	const closeFileHandle = async (handle) => {
		try { await handle?.close(); } catch { /* already closed */ }
	};
	const noFollowFlag = fsConstants.O_NOFOLLOW ?? 0;
	const nonBlockFlag = fsConstants.O_NONBLOCK ?? 0;
	const directoryFlag = fsConstants.O_DIRECTORY ?? 0;
	const openWorkspaceFile = async (candidate) => {
		const roots = workspaceRootRealpaths();
		if (roots.length === 0 || typeof candidate !== "string" || candidate === "") return null;
		const requested = resolve(normalizeServerPath(candidate));
		let canonical;
		try { canonical = realpathSync(requested); } catch { return null; }
		let handle;
		try {
			handle = await openFile(canonical, fsConstants.O_RDONLY | nonBlockFlag | noFollowFlag);
			const stats = await handle.stat();
			if (!stats.isFile()) {
				await closeFileHandle(handle);
				return null;
			}
			const actual = actualPathOfHandle(handle);
			if (!actual || !pathWithinWorkspaceRoots(actual, roots)) {
				await closeFileHandle(handle);
				return null;
			}
			return { handle, path: actual, stats, roots };
		} catch {
			await closeFileHandle(handle);
			return null;
		}
	};
	const openWorkspaceDirectory = async (candidate) => {
		const roots = workspaceRootRealpaths();
		if (roots.length === 0 || typeof candidate !== "string" || candidate === "") return null;
		const requested = resolve(normalizeServerPath(candidate));
		let canonical;
		...
		let handle;
		try {
			handle = await openFile(canonical, fsConstants.O_RDONLY | directoryFlag | noFollowFlag);
			const stats = await handle.stat();
	...
	const relativeDescriptorPath = (handle, name) => {
		const base = descriptorPath(handle);
	...
	const handleApi = async (req, res, url, rest) => {
		...
		const operationError = (err) => {
			const code = err instanceof GitOperationError && typeof err.code === "string" ? err.code : (typeof err?.code === "string" ? err.code : "git-operation-failed");
			const status = ({
				...
				"state-changed": 409,
				"repository-busy": 409,
				"provider-ambiguous": 503,
				"provider-incompatible": 503,
				"provider-unavailable": 503,
				"unknown-result": 409,
				"conflicted": 409,
				"change-set-expired": 409,
				"change-set-too-large": 413,
				"hunk-stale": 409,
				"nothing-to-commit": 409,
				"identity-unavailable": 409,
				"confirmation-required": 400,
				"branch-exists": 409,
				"branch-not-found": 404,
				"protected-switch": 409,
				"target-changed": 409,
				"target-required": 400,
				"upstream-required": 409,
				...
				"remote-invalid-response": 502,
				"provider-invalid-response": 502,
				"no-intermediate-state": 409,
				"unsupported-intermediate-state": 409,
				"abort-incomplete": 409,
				"client-incompatible": 409,
				"not-git-repository": 404,
				"workspace-not-allowed": 403,
			})[code] ?? 400;
			const detail = err instanceof GitOperationError ? (err.publicDetail ?? err.message) : undefined;
			error(res, status, code, detail, { errorCode: code, retryable: err?.retryable === true, requiresRefresh: err?.requiresRefresh === true, ...(Array.isArray(err?.nextActions) ? { nextActions: err.nextActions } : {}) });
		};
		const operationDetail = /^\/git\/operations\/([^/]+)$/.exec(rest);
		const operationCancel = /^\/git\/operations\/([^/]+)\/cancel$/.exec(rest);
		if (method === "POST" && ["/git/change-sets", "/git/stage", "/git/unstage", "/git/commit/preflight", "/git/commit", "/git/branches/preflight", "/git/branches", "/git/branch-switch/preflight", "/git/branch-switch", "/git/branch-rename"].includes(rest)) {
			try {
				if (rest === "/git/change-sets" || rest === "/git/commit/preflight") {
					if (requirePost(method, res)) return;
					const body = await readJson(req, res);
					if (body === undefined) return;
					if (!body || typeof body !== "object" || Array.isArray(body)) return error(res, 400, "invalid-argument", "request body must be an object");
					const allowed = rest === "/git/change-sets" ? new Set(["repositoryId", "kind"]) : new Set(["repositoryId", "message"]);
					if (rejectUnknownKeys(body, allowed, res)) return;
					if (rest === "/git/change-sets") {
						const value = await gitWrites.createChangeSet({ repositoryId: body.repositoryId, kind: body.kind });
						sendJson(res, 200, { ok: true, changeSet: value });
					} else {
						const value = await gitWrites.commitPreflight({ repositoryId: body.repositoryId, message: body.message });
						sendJson(res, 200, { ok: true, preflight: value });
					}
					return;
				}
				const branchRoutes = new Set(["/git/branches/preflight", "/git/branches", "/git/branch-switch/preflight", "/git/branch-switch", "/git/branch-rename"]);
				if (branchRoutes.has(rest)) {
					const body = await readJson(req, res, 4 * 1024 * 1024);
					if (body === undefined) return;
					if (!body || typeof body !== "object" || Array.isArray(body)) return error(res, 400, "invalid-argument", "request body must be an object");
					const allowed = rest.endsWith("/preflight")
						? rest === "/git/branch-switch/preflight"
							? new Set(["repositoryId", "targetRef", "targetBranch", "localName"])
							: new Set(["repositoryId", "action", "name", "newName", "oldName", "startOid", "remoteRef"])
						: new Set(["repositoryId", "requestId", "params", "preconditionToken"]);
					if (rejectUnknownKeys(body, allowed, res)) return;
					if (rest.endsWith("/preflight")) {
						// Branch creation/rename are the only actions accepted by this preflight.
						const action = rest === "/git/branch-switch/preflight" ? "switch" : body.action;
						if (rest === "/git/branches/preflight" && !["create", "rename"].includes(action)) return error(res, 400, "invalid-argument", "action must be create or rename");
						const value = await gitWrites.branchPreflight({ ...body, action });
						sendJson(res, 200, { ok: true, preflight: value });
					} else {
						const action = rest === "/git/branch-switch" ? "switch" : rest === "/git/branch-rename" ? "rename" : "create";
						if (!body.params || typeof body.params !== "object" || Array.isArray(body.params)) return error(res, 400, "invalid-argument", "params must be the preflight params object");
						const params = body.params;
						const value = await gitWrites.submitBranch({ repositoryId: body.repositoryId, requestId: body.requestId, actor: "shared-auth-token", action, params, preconditionToken: body.preconditionToken });
						sendJson(res, 202, acceptedOperationResponse(value, `/git/operations/${encodeURIComponent(value.operationId)}`));
					}
					return;
				}
				if (requirePost(method, res)) return;
				...
				if (body === undefined) return;
				if (!body || typeof body !== "object" || Array.isArray(body)) return error(res, 400, "invalid-argument", "request body must be an object");
				const allowed = rest === "/git/stage" || rest === "/git/unstage"
					? new Set(["repositoryId", "requestId", "changeSetId", "selections", "preconditionToken"])
					: new Set(["repositoryId", "requestId", "message", "preconditionToken", "confirm"]);
				if (rejectUnknownKeys(body, allowed, res)) return;
				if (rest === "/git/stage" || rest === "/git/unstage") {
					const value = await gitWrites.submitStage({ repositoryId: body.repositoryId, requestId: body.requestId, actor: "shared-auth-token", changeSetId: body.changeSetId, selections: body.selections, preconditionToken: body.preconditionToken, unstage: rest === "/git/unstage" });
					sendJson(res, 202, acceptedOperationResponse(value, `/git/operations/${encodeURIComponent(value.operationId)}`));
				} else {
					const value = await gitWrites.submitCommit({ repositoryId: body.repositoryId, requestId: body.requestId, actor: "shared-auth-token", message: body.message, preconditionToken: body.preconditionToken, confirm: body.confirm });
					sendJson(res, 202, acceptedOperationResponse(value, `/git/operations/${encodeURIComponent(value.operationId)}`));
				}
			} catch (err) { operationError(err); }
			return;
		}
		const b3PreflightRoutes = new Set(["/git/fetch/preflight", "/git/pull/preflight", "/git/push/preflight", "/git/sync/preflight", "/git/abort/preflight"]);
		const b3SubmitRoutes = new Set(["/git/fetch", "/git/pull", "/git/push", "/git/sync", "/git/abort", "/git/confirmations"]);
		if (b3PreflightRoutes.has(rest) || b3SubmitRoutes.has(rest)) {
			if (requirePost(method, res)) return;
			if (requireGitContract(req, res)) return;
			try {
				const body = await readJson(req, res, 4 * 1024 * 1024);
				if (body === undefined) return;
				if (!body || typeof body !== "object" || Array.isArray(body)) return error(res, 400, "invalid-argument", "request body must be an object");
				const b3Allowed = rest === "/git/confirmations"
					? new Set(["repositoryId", "confirmationRequestId", "operationType", "params", "preconditionToken"])
					: rest === "/git/abort/preflight"
						? new Set(["repositoryId"])
						: b3PreflightRoutes.has(rest)
							? new Set(["repositoryId", "remote", "branch", "targetBranch", "localBranch", "sourceBranch", "strategy", "setUpstream", "force", "correlationId"])
							: new Set(["repositoryId", "requestId", "params", "preconditionToken", "challengeId"]);
				if (rejectUnknownKeys(body, b3Allowed, res)) return;
				if (rest === "/git/confirmations") {
					const value = await gitRemote.issueConfirmation({ repositoryId: body.repositoryId, confirmationRequestId: body.confirmationRequestId, actor: gitActor(), operationType: body.operationType ?? "git.abort", params: body.params, preconditionToken: body.preconditionToken });
					sendJson(res, 200, { ok: true, ...value });
					return;
				}
				if (b3PreflightRoutes.has(rest)) {
					let value;
					if (rest === "/git/fetch/preflight") value = await gitRemote.fetchPreflight({ repositoryId: body.repositoryId, remote: body.remote, branch: body.branch, targetBranch: body.targetBranch, setUpstream: body.setUpstream, force: body.force, correlationId: body.correlationId });
					else if (rest === "/git/pull/preflight") value = await gitRemote.pullPreflight({ repositoryId: body.repositoryId, remote: body.remote, branch: body.branch, targetBranch: body.targetBranch, localBranch: body.localBranch, sourceBranch: body.sourceBranch, strategy: body.strategy, setUpstream: body.setUpstream, force: body.force, correlationId: body.correlationId });
					else if (rest === "/git/push/preflight") value = await gitRemote.pushPreflight({ repositoryId: body.repositoryId, remote: body.remote, branch: body.branch, targetBranch: body.targetBranch, localBranch: body.localBranch, sourceBranch: body.sourceBranch, setUpstream: body.setUpstream, force: body.force, correlationId: body.correlationId });
					else if (rest === "/git/sync/preflight") value = await gitRemote.syncPreflight({ repositoryId: body.repositoryId, remote: body.remote, branch: body.branch, targetBranch: body.targetBranch, localBranch: body.localBranch, sourceBranch: body.sourceBranch, strategy: body.strategy, setUpstream: body.setUpstream, force: body.force, correlationId: body.correlationId });
					else value = await gitRemote.abortPreflight({ repositoryId: body.repositoryId });
					sendJson(res, 200, { ok: true, preflight: value });
					return;
				}
				if (!body.params || typeof body.params !== "object" || Array.isArray(body.params)) return error(res, 400, "invalid-argument", "params must be the preflight params object");
				const input = { repositoryId: body.repositoryId, requestId: body.requestId, actor: gitActor(), params: body.params, preconditionToken: body.preconditionToken };
				let value;
				if (rest === "/git/fetch") value = await gitRemote.submitFetch(input);
				else if (rest === "/git/pull") value = await gitRemote.submitPull(input);
				else if (rest === "/git/push") value = await gitRemote.submitPush(input);
				else if (rest === "/git/sync") value = await gitRemote.submitSync(input);
				else value = await gitRemote.submitAbort({ ...input, challengeId: body.challengeId });
				sendJson(res, 202, acceptedOperationResponse(value, `/git/operations/${encodeURIComponent(value.operationId)}`));
			} catch (err) { operationError(err); }
			return;
		}
		const handoffRoute = /^\/git\/operations\/([^/]+)\/handoff$/.exec(rest) || (rest === "/git/handoff" ? [null, null] : null);
		if (handoffRoute) {
			if (requirePost(method, res)) return;
			if (requireGitContract(req, res)) return;
			try {
				...
				if (body === undefined) return;
				if (!body || typeof body !== "object" || Array.isArray(body)) return error(res, 400, "invalid-argument", "request body must be an object");
				if (rejectUnknownKeys(body, new Set(["repositoryId", "operationId", "target", "reason", "requestId", "expectedRevision", "facts"]), res)) return;
				const operationId = handoffRoute[1] ? decodeURIComponent(handoffRoute[1]) : body.operationId;
				const value = await gitRemote.handoff({ repositoryId: body.repositoryId, operationId, target: body.target, reason: body.reason, requestId: body.requestId, expectedRevision: body.expectedRevision, facts: body.facts });
				sendJson(res, 200, { ok: true, operation: value });
			} catch (err) { operationError(err); }
			return;
		}
		if (rest === "/git/recovery/acknowledge") {
			try {
				if (requirePost(method, res)) return;
				const body = await readJson(req, res);
				if (body === undefined) return;
				if (!body || typeof body !== "object" || Array.isArray(body)) return error(res, 400, "invalid-argument", "request body must be an object");
				if (typeof body.repositoryId !== "string" || body.repositoryId === "" || typeof body.operationId !== "string" || body.operationId === "" || typeof body.requestId !== "string" || !Number.isInteger(body.expectedRevision) || !body.facts || typeof body.facts !== "object" || Array.isArray(body.facts) || body.facts.confirmed !== true || typeof body.facts.stateVersion !== "string" || !Number.isFinite(body.facts.observedAt)) return error(res, 400, "invalid-argument", "repositoryId, operationId, requestId, expectedRevision and confirmed facts are required");
				await gitOperations.ready?.();
				const value = await gitOperations.acknowledgeRecovery({
		...
		if (rest === "/git/operations" || operationDetail || operationCancel) {
			try {
				await gitOperations.ready?.();
				if (operationCancel) {
					if (requirePost(method, res)) return;
					...
					if (body === undefined) return;
					if (!body || typeof body !== "object" || Array.isArray(body)) return error(res, 400, "invalid-argument", "request body must be an object");
					const value = await gitOperations.cancel(decodeURIComponent(operationCancel[1]), {
						requestId: body.requestId,
						expectedRevision: body.expectedRevision,
					});
					sendJson(res, 202, acceptedOperationResponse(value, `/git/operations/${encodeURIComponent(value.operationId)}`));
				...
				if (operationDetail) {
					const value = gitOperations.get(decodeURIComponent(operationDetail[1]));
					if (!value) return error(res, 404, "operation-not-found");
					sendJson(res, 200, { ok: true, operation: value });
					return;
				}
				const value = gitOperations.list({
		...
		if (rest === "/bootstrap") {
			...
			sendJson(res, 200, {
				...
				git: { ...gitCapabilities(), operations: gitOperations.capabilities?.() ?? null },
				agents: agentList,
		...
		if (rest === "/git/capabilities") {
			if (requireGet(method, res)) return;
			sendJson(res, 200, { ok: true, git: { ...gitCapabilities(), operations: gitOperations.capabilities?.() ?? null } });
			return;
		}
		const gitMatch = /^\/git\/(context|status|branches|remotes|graph|commit|diff)$/.exec(rest);
		if (gitMatch) {
			if (requireGet(method, res)) return;
			const op = gitMatch[1];
			try {
				if (op === "context") {
					...
					value.capabilities = gitCapabilities();
					sendJson(res, 200, { ok: true, ...value });
				} else {
					...
					else if (op === "remotes") value = await gitRemote.remotes(repositoryId);
					else if (op === "graph") {
						let refs;
			...
			const code = ["workspace-not-allowed", "not-git-repository", "git-provider-unavailable", "provider-incompatible", "provider-invalid-response", "remote-invalid-response", "remote-error", "invalid-oid", "graph-tip-invalid", "graph-too-many-tips", "graph-stale"].includes(e?.code) ? e.code : "git-command-failed";
			const publicCode = code === "invalid-oid" ? "bad-request" : code;
			const status = e?.status ?? (publicCode === "workspace-not-allowed" ? 403 : publicCode === "git-provider-unavailable" || publicCode === "provider-incompatible" ? 503 : publicCode === "not-git-repository" ? 404 : publicCode === "provider-invalid-response" || publicCode === "remote-invalid-response" || publicCode === "remote-error" ? 502 : publicCode === "bad-request" ? 400 : 400);
				error(res, status, publicCode, ["git-command-failed", "remote-error"].includes(publicCode) ? undefined : e?.message);
			}
			return;
		}

		// ── App 自动更新：主机源（updateDir + manifest.json，manifest 为唯一权威） ──
		if (rest === "/update/manifest") {
			if (requireGet(method, res)) return;
			const dir = resolveUpdateDir(config);
			const r = readUpdateManifest(dir);
			if (!r.ok) {
				error(res, r.error === "update-not-configured" ? 503 : 404, r.error);
				return;
			}
			sendJson(res, 200, { ok: true, manifest: r.manifest });
			return;
		}
		...
		if (rest === "/update/apk") {
			...
			const file = apkPathInDir(dir, r.manifest.apk);
			if (file === "" || !existsSync(file)) {
				error(res, 404, "update-apk-missing");
				return;
			}
			// 按 manifest 的 sha256 校验再下发（防目录内文件被换/传输损坏——成本一次读盘）
			if (typeof r.manifest.sha256 === "string" && r.manifest.sha256 !== "") {
				try {
					const actual = createHash("sha256").update(readFileSync(file)).digest("hex");
					if (actual !== r.manifest.sha256.toLowerCase()) {
						error(res, 500, "update-apk-checksum-mismatch");
						return;
					}
				} catch {
					error(res, 500, "update-apk-read-failed");
					return;
				}
			}
			res.writeHead(200, {
			...
			stream.on("error", () => {
				if (!res.headersSent) error(res, 500, "update-apk-read-failed");
				else res.destroy();
			});
			stream.pipe(res);
			return;
			...
			if (!target && sessionId) {
				...
				try {
					...
					if (query?.readSession) {
						...
						folded = foldFromEvents(snap?.events ?? []);
					}
			...
			try {
				...
				try {
					if (typeof body.model === "string" && body.model !== "") {
						...
						//
						// Issue #14：默认模型必须取自内核的 `agentDefaultModel` 服务
						// （currentSelection() —— 语义即"没有会话级选择时的默认模型"）。
						// 曾只读 readSessionConfig(sessionId)，那是**新建会话自己**的 session.control
						// 投影，全新会话必然为空 → 恒定落到 400，且文案把"读不到"误报成"未配置"。
						// 该服务可能不存在（旧宿主）→ 可选获取并回退，避免把兼容性修复变成新的兼容问题。
						const defaults = (() => {
						...
						const provider = (typeof body.provider === "string" && body.provider !== "" ? body.provider : undefined)
							?? (typeof defaults?.provider === "string" && defaults.provider !== "" ? defaults.provider : undefined)
							?? (typeof config.provider === "string" && config.provider !== "" ? config.provider : undefined)
							?? "deepseek-official";
						const model = (typeof defaults?.model === "string" && defaults.model !== "" ? defaults.model : undefined)
							?? (typeof config.model === "string" && config.model !== "" ? config.model : undefined);
						if (typeof model === "string" && model !== "") {
							const effort = body.reasoningEffort ?? defaults?.reasoningEffort ?? config.reasoningEffort;
							try {
								await apiRpc("session.selectModel", {
									sessionId,
									provider,
		...
		if (rest === "/history") {
			...
			const surface = eventsOf(session).filter((event) => SURFACE_TYPES.has(event.type));
			let events;
			if (afterParam !== null) {
		...
		if (rest === "/catalog") {
			...
			// 图像链路：限额从内核 session/control 的基线投影取（与 PC 端同一组数字），取不到用内核默认。
		...
		if (rest === "/llm-providers/probe") {
			...
			try {
				...
				try {
					// 字段名须与内核 LlmModelDiscoveryRequest 契约一致：{ provider?, baseURL?, api?, apiKey? }。
					// 曾误用 protocol / credential —— 二者不在契约内，TS 接口不做运行期校验，
					// 多余字段被静默丢弃：用户填的 Key 根本发不出去，需鉴权的提供商必然 401（Issue #13）。
					discovered = await llm.discoverModels(ns, {
						baseURL,
						...(typeof body.protocol === "string" && body.protocol !== "" ? { api: body.protocol } : {}),
						...(apiKey !== "" ? { apiKey } : {}),
					});
				} catch (err) {
		...
		if (rest === "/push-test") {
			...
			for (const target of config.pushUrls) {
				try {
					await pushToChannel(target, "test", "🔔 测试通知", "配置验证", "收到即说明该通道配置正确（来自 DSH Remote）", "");
					results.push({ name: target.name, format: target.format, ok: true });
				} catch (err) {
					results.push({ name: target.name, format: target.format, ok: false, error: err?.message ?? String(err) });
				}
			}
		...
		if (rest === "/respond") {
			...
			if (body?.kind === "question") {
				...
				const qById = qPhoneId ? pendingQuestions.get(qPhoneId) : undefined;
				const qByRpc = rpcId ? [...pendingQuestions.values()].find((e) => e.rpcId === rpcId) : undefined;
				if (qById && qByRpc && qById !== qByRpc) return error(res, 400, "interaction-mismatch");
				const qEntry = qById ?? qByRpc;
				if (qEntry) {
					if (qPhoneId && qPhoneId !== qEntry.phoneId) return error(res, 400, "question-mismatch");
					if (typeof body?.sessionId !== "string" || body.sessionId !== qEntry.sessionId) return error(res, 400, "session-mismatch");
					...
					if (!await finishQuestion(qEntry, { answers: body.answers })) return error(res, 503, "respond-unavailable", "问询结算失败，请重试");
					sendJson(res, 200, { ok: true, accepted: true });
					return;
				}
				if (!Array.isArray(body?.answers)) return error(res, 400, "bad-request", "question respond expects answers[]");
				...
				const aById = aPhoneId ? pendingApprovals.get(aPhoneId) : undefined;
				const aByRpc = rpcId ? [...pendingApprovals.values()].find((e) => e.rpcId === rpcId) : undefined;
				if (aById && aByRpc && aById !== aByRpc) return error(res, 400, "interaction-mismatch");
				const aEntry = aById ?? aByRpc;
				if (aEntry) {
					if (typeof body?.sessionId !== "string" || body.sessionId !== aEntry.sessionId) return error(res, 400, "session-mismatch");
					if (aPhoneId && aPhoneId !== aEntry.phoneId) return error(res, 400, "approval-mismatch");
					if (outcome !== "allowed-once" && outcome !== "rejected") return error(res, 400, "approval-outcome-invalid");
					if (!await finishApproval(aEntry, outcome)) return error(res, 503, "respond-unavailable", "审批结算失败，请重试");
					sendJson(res, 200, { ok: true, accepted: true });
					return;
				}
			} else if (body?.kind === "cancel") {
				if (!rpcId) return error(res, 400, "bad-request", "missing rpcId");
				const cancelSessionId = typeof body?.sessionId === "string" ? body.sessionId : "";
				if (cancelSessionId === "") return error(res, 400, "session-required", "cancel requires sessionId");
				const cancelEntry = [...pendingApprovals.values()].find((e) => e.rpcId === rpcId)
					?? [...pendingQuestions.values()].find((e) => e.rpcId === rpcId);
				if (cancelEntry) {
					if (cancelSessionId !== cancelEntry.sessionId) return error(res, 400, "session-mismatch");
					const accepted = pendingApprovals.has(cancelEntry.phoneId)
						? await finishApproval(cancelEntry, "cancelled")
						: await finishQuestion(cancelEntry, questionCancelledError(), { reject: true });
					if (!accepted) return error(res, 503, "respond-unavailable", "交互取消失败，请重试");
					sendJson(res, 200, { ok: true, accepted: true });
					return;
				}
			}
			// 走到这里说明本地 answerer 无匹配条目：只有旧代宿主的 apiProxy 通道还能兜底。
			// 新代宿主上该服务不存在（ADR 0001 弃用通道）——明确报错并带升级指引，
			// 而不是让使用者面对一句「内核 apiProxy 不可用」的含糊失败。
			if (!proxy || typeof proxy.respond !== "function") {
				return error(res, 503, "respond-unavailable", `问询/审批通道不可用：宿主未提供 Remote 事件结算或旧代 apiProxy。${HOST_UPGRADE_HINT}`);
			}
			if (!rpcId) return error(res, 400, "bad-request", "missing rpcId");
			let message;
			if (body?.kind === "question") {
		...
		if (rest === "/diagnostics") {
			...
			// apiProxy remains visible only when the legacy frame bridge is actually active.
			if (proxy && typeof proxy.respond === "function") services.apiProxy = true;
			...
			// 问询/审批桥状态：旧 apiProxy 与 RC1 $events 均明确呈现。
			if (proxy && typeof proxy.respond === "function") {
			...
			const approvalNote = approvalMode === "both" && eventsCapable()
				...
				: approvalMode === "both"
					? "approvalMode=both 但 $events 通道不可用 → 按 mobile 语义降级（手机在线独占，桌面不弹）"
			...
			sendJson(res, 200, {
				...
				services,
				checks,
				// ADR 0001（Issue #9）：宿主节——版本号用于身份识别，能力结果用于行为判断，两者不可互相替代。
				// App 诊断页据 supported 显示兼容状态；capabilities 缺失项即对应功能的失败原因。
				host: hostReport(),
				notes: [
					approvalNote,
					...
					// 宿主代际与能力（ADR 0001）：只报事实与缺失原因，不代替使用者做决定。
					hostNote(),
				],
			});
			return;
		...
		if (rest === "/files" && method === "GET") {
			if (!authEnabled) return error(res, 401, "files-auth-required", "文件接口必须启用 authToken");
			if (!descriptorFsAvailable) return error(res, 503, "files-unavailable", "当前平台不支持安全的 descriptor-relative 文件传输");
			const requestedPath = url.searchParams.get("path");
			if (!requestedPath || requestedPath === "") return error(res, 400, "bad-request", "missing path");
			// Open and validate the object before sending headers. The descriptor is
			// then the authority for the stream, so a later rename/symlink swap cannot
			// redirect the download outside the registered workspace.
			const target = await openWorkspaceFile(requestedPath);
			if (!target) return error(res, 404, "file-not-found");
			const name = basename(target.path);
			let stream;
			try {
				stream = target.handle.createReadStream({ autoClose: true });
				guardRes(res);
				stream.once("error", (err) => {
					// ReadStream errors are asynchronous and therefore cannot be caught
					// by the surrounding try/catch. Always consume the error so a failed
					// download cannot crash the DSH host.
					ctx.logger.warn?.(`mobile-remote: file download failed (${requestedPath}): ${err?.message ?? err}`);
				...
				else await closeFileHandle(target.handle);
				if (!res.headersSent) return error(res, 404, "file-not-found", err?.message ?? String(err));
				if (!res.destroyed) res.destroy();
			}
			return;
		}
		if (rest === "/files/upload" && method === "POST") {
			if (!authEnabled) return error(res, 401, "files-auth-required", "文件接口必须启用 authToken");
			if (!descriptorFsAvailable) return error(res, 503, "files-unavailable", "当前平台不支持安全的 descriptor-relative 文件传输");
			// { sessionId?, name, data(base64) } → 写入目标会话工作目录（无 sessionId 时写首个工作区根）
			const body = await readJson(req, res, 64 * 1024 * 1024);
			if (body === undefined) return;
			const name = typeof body?.name === "string" ? body.name.trim() : "";
			...
			if (!/^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/.test(body.data)) {
				return error(res, 400, "invalid-base64", "data 必须是标准 base64");
			}
			let dir;
			const sessionId = typeof body?.sessionId === "string" && body.sessionId !== "" ? body.sessionId : undefined;
			if (sessionId) {
			...
			const directory = typeof dir === "string" ? await openWorkspaceDirectory(dir) : null;
			if (!directory) return error(res, 404, "workspace-not-found", "无法确定授权工作区目录");
			const data = Buffer.from(body.data, "base64");
			if (data.length === 0) {
				await closeFileHandle(directory.handle);
				return error(res, 400, "bad-request", "empty payload");
			}
			let fileHandle;
			let targetPath;
			try {
				// On POSIX this is descriptor-relative: the validated directory fd
				// remains attached to the original directory across rename/symlink races.
				targetPath = relativeDescriptorPath(directory.handle, name);
				if (!targetPath) return error(res, 503, "files-unavailable", "当前平台不支持安全的 descriptor-relative 文件传输");
				fileHandle = await openFile(targetPath, fsConstants.O_WRONLY | fsConstants.O_CREAT | fsConstants.O_EXCL | noFollowFlag, 0o600);
				const stats = await fileHandle.stat();
				const actual = actualPathOfHandle(fileHandle);
				if (!stats.isFile() || !actual || !pathWithinWorkspaceRoots(actual, directory.roots)) {
					// Do not unlink by pathname after closing the descriptor: a local racer
					// could replace the name and make cleanup delete an unrelated file.
					// The file is still empty because the boundary check precedes the write.
					return error(res, 403, "workspace-boundary");
				}
				await fileHandle.writeFile(data);
				sendJson(res, 200, { ok: true, path: actual, bytes: data.length });
			} catch (err) {
				// A partially written upload must not masquerade as a complete file.
				// Truncate through the same descriptor (never by pathname) so the
				// failed attempt leaves an obviously empty object behind.
				if (fileHandle) await fileHandle.truncate(0).catch(() => {});
				return error(res, err?.code === "EEXIST" ? 409 : 400, err?.code === "EEXIST" ? "file-exists" : "write-failed", err?.message ?? String(err));
			} finally {
	...
	const settledEventIds = new Map(); // eventId -> timestamp; replayed $events frames must not recreate cards
	...
	/**
	 * 事件桥就绪判定。
	 * ADR 0001：`dispatchRpc` 是网关的**未文档化私有方法**（类型声明里只有 public 的
	 * wireStream/registerRemoteEvents/invoke/stream），是问询/审批**出站结算**的唯一通路；
	 * 上游若变更该通路，这里会自然变 false → 手机端不再弹卡（而不是弹出后无法应答）。
	 * 诊断的 checks.remoteEvents 即本判定的对外投影。
	 */
	const eventsCapable = () => {
		if (!eventsState.ready || eventsAbort === null || eventsAbort.signal.aborted) return false;
		try {
			const gateway = ctx.get("typertGateway");
			return !!gateway && typeof gateway.dispatchRpc === "function";
		} catch {
			return false;
		}
	};
	...
	const questionAbortedError = () => questionCancelledError(
		"ask_user_question was aborted before the user answered",
		"ASK_ABORTED",
	);

	/** 结算回执：与浏览器 GUI 完全相同的 $events/result 通道（网关 dispatchRpc 拦截解析）。 */
	const postEventsOutcome = async (eventId, outcome, clientId = eventsState.clientId) => {
		try {
			const gateway = ctx.get("typertGateway");
			if (!gateway || typeof gateway.dispatchRpc !== "function" || !clientId || !eventsState.ready || clientId !== eventsState.clientId) return false;
			const full = await gateway.dispatchRpc(TYPERT_REMOTE_EVENT_RESULT, {
	...
	const postEventsValue = (eventId, value, clientId) => postEventsOutcome(eventId, { kind: "result", value }, clientId);
	const postEventsNext = (eventId, clientId) => postEventsOutcome(eventId, { kind: "next" }, clientId);
	const postEventsRejection = (eventId, reason, clientId) => postEventsOutcome(eventId, {
		kind: "rejected",
		error: {
			name: typeof reason?.name === "string" && reason.name !== "" ? reason.name : "UserQuestionError",
			message: typeof reason?.message === "string" && reason.message !== "" ? reason.message : "the user cancelled ask_user_question",
			...(typeof reason?.code === "string" && reason.code !== "" ? { code: reason.code } : { code: "ASK_CANCELLED" }),
		},
	}, clientId);
	// A pending interaction is removed only after its host settlement succeeds.
	// This keeps a failed $events/result retryable and gives every entry a small
	// compare-and-set guard against phone/desktop/timeout races.
	const rememberSettledEvent = (eventId) => {
		if (typeof eventId !== "string" || eventId === "") return;
		settledEventIds.set(eventId, Date.now());
		while (settledEventIds.size > 2048) settledEventIds.delete(settledEventIds.keys().next().value);
	};
	const completeApproval = (entry, { cancelled = false } = {}) => {
		if (pendingApprovals.get(entry.phoneId) !== entry) return settledEventIds.has(entry.rpcId);
		pendingApprovals.delete(entry.phoneId);
		pendingFrameDelete("a", entry.rpcId);
		rememberSettledEvent(entry.rpcId);
		if (entry.timer) { clearTimeout(entry.timer); entry.timer = null; }
		clearAbortWatch(entry);
		ctx.logger.info?.(`mobile-remote: approval/request 已完成（${entry.phoneId}）`);
		broadcast({ type: "mobile/frame", frame: { type: "approval/resolved", approvalId: entry.phoneId, rpcId: entry.rpcId, ...(cancelled ? { cancelled: true } : {}) } });
		return true;
	};
	const completeQuestion = (entry, { cancelled = false } = {}) => {
		if (pendingQuestions.get(entry.phoneId) !== entry) return settledEventIds.has(entry.rpcId);
		pendingQuestions.delete(entry.phoneId);
		pendingFrameDelete("q", entry.rpcId);
		rememberSettledEvent(entry.rpcId);
		if (entry.timer) { clearTimeout(entry.timer); entry.timer = null; }
		clearAbortWatch(entry);
		ctx.logger.info?.(`mobile-remote: user-questions/request 已完成（${entry.phoneId}）`);
		broadcast({ type: "mobile/frame", frame: { type: "question/resolved", questionRpcId: entry.rpcId, questionId: entry.phoneId, ...(cancelled ? { cancelled: true } : {}) } });
		return true;
	};
	// The Gateway removes a disconnected $events client delivery but cannot send
	// that client the later desktop cancel. Drop only entries created by that
	// stream (not local mobile-held waterfall entries), otherwise a stale phone
	// card would survive until the 120s timeout after a desktop win.
	const discardRemotePending = () => {
		for (const entry of [...pendingApprovals.values()]) {
			if (!entry.remote || entry.settling || entry.abortSignal || pendingApprovals.get(entry.phoneId) !== entry) continue;
			pendingApprovals.delete(entry.phoneId);
			pendingFrameDelete("a", entry.rpcId);
			if (entry.timer) { clearTimeout(entry.timer); entry.timer = null; }
			clearAbortWatch(entry);
			broadcast({ type: "mobile/frame", frame: { type: "approval/resolved", approvalId: entry.phoneId, rpcId: entry.rpcId, cancelled: true } });
		}
		for (const entry of [...pendingQuestions.values()]) {
			if (!entry.remote || entry.settling || entry.abortSignal || pendingQuestions.get(entry.phoneId) !== entry) continue;
			pendingQuestions.delete(entry.phoneId);
			pendingFrameDelete("q", entry.rpcId);
			if (entry.timer) { clearTimeout(entry.timer); entry.timer = null; }
			clearAbortWatch(entry);
			broadcast({ type: "mobile/frame", frame: { type: "question/resolved", questionRpcId: entry.rpcId, questionId: entry.phoneId, cancelled: true } });
		}
	};
	const rearmRemoteTimeout = (entry) => {
		if (!entry.remote || typeof entry.onTimeout !== "function") return;
		if (pendingApprovals.get(entry.phoneId) !== entry && pendingQuestions.get(entry.phoneId) !== entry) return;
		// Keep retrying until the receipt lands or the timeout path handles it;
		// a transient failure must never leave a pending entry without a timer.
		armTimeout(entry, entry.onTimeout);
	};
	// A disposed agent can no longer receive its interaction: end the local hold
	// and clear the phone card instead of leaving a stale entry until the timeout.
	const closePendingForSession = (sessionId) => {
		for (const entry of [...pendingApprovals.values()]) {
			if (entry.sessionId !== sessionId || entry.settling) continue;
			if (entry.remote) {
				pendingApprovals.delete(entry.phoneId);
				pendingFrameDelete("a", entry.rpcId);
				if (entry.timer) { clearTimeout(entry.timer); entry.timer = null; }
			...
			void finishApproval(entry, "cancelled");
		}
		for (const entry of [...pendingQuestions.values()]) {
			if (entry.sessionId !== sessionId || entry.settling) continue;
			if (entry.remote) {
			...
			void finishQuestion(entry, questionAbortedError(), { reject: true });
		}
	};
	const finishApproval = async (entry, outcome) => {
		// A desktop $events client may win between the lookup and this call. Its
		// cancel frame already records the event as settled, so the mobile retry is
		// an idempotent success rather than a misleading 503.
		if (pendingApprovals.get(entry.phoneId) !== entry || entry.settling) return settledEventIds.has(entry.rpcId);
		entry.settling = true;
		try {
			const result = await entry.settle(outcome);
			if (result === false) {
				entry.settling = false;
				if (entry.timedOut) {
					// This plugin's own Gateway delivery is gone (stream disconnected
					// mid-flight), so no receipt from this client can ever succeed.
					// Forcing the close only retires OUR delivery: the Host waterfall
					// keeps running and is answered by whichever $events client is still
					// connected (desktop GUI), exactly like a desktop-first win. Keeping
					// the entry would instead leave a stale phone card until a new stream.
					ctx.logger.warn?.(`mobile-remote: approval/request 结算回执丢失（${entry.phoneId}）→ 本端投递失效，收卡并交由其它客户端应答`);
					rememberSettledEvent(entry.rpcId);
					completeApproval(entry, { cancelled: true });
				} else {
					rearmRemoteTimeout(entry);
				}
				return false;
			}
		} catch (err) {
			entry.settling = false;
			rearmRemoteTimeout(entry);
			ctx.logger.warn?.(`mobile-remote: approval/request 结算失败（${entry.phoneId}）：${err?.message ?? err}`);
			return false;
		}
		return completeApproval(entry);
	};
	const finishQuestion = async (entry, value, { reject = false } = {}) => {
		if (pendingQuestions.get(entry.phoneId) !== entry || entry.settling) return settledEventIds.has(entry.rpcId);
		const settle = reject ? entry.reject : entry.settle;
		...
		try {
			...
			if (result === false) {
				entry.settling = false;
				if (entry.timedOut) {
					// Same as the approval path: retire only this client's delivery and
					// leave the Host waterfall to another connected $events client.
					ctx.logger.warn?.(`mobile-remote: user-questions/request 结算回执丢失（${entry.phoneId}）→ 本端投递失效，收卡并交由其它客户端应答`);
					rememberSettledEvent(entry.rpcId);
					completeQuestion(entry, { cancelled: true });
				} else {
					rearmRemoteTimeout(entry);
				}
				return false;
			...
			entry.settling = false;
			rearmRemoteTimeout(entry);
			ctx.logger.warn?.(`mobile-remote: user-questions/request 结算失败（${entry.phoneId}）：${err?.message ?? err}`);
			return false;
		}
		return completeQuestion(entry);
	};
	const armTimeout = (entry, onFire) => {
		if (entry.timer) clearTimeout(entry.timer);
		entry.onTimeout = onFire;
	...
	const watchAbort = (entry, signal, onAbort) => {
	...
	const clearAbortWatch = (entry) => {
		if (!entry?.abortSignal || typeof entry.onAbort !== "function") return;
		try { entry.abortSignal.removeEventListener("abort", entry.onAbort); } catch {}
		entry.abortSignal = null;
		entry.onAbort = null;
	};
	...
	// 超时 fail-close。rpcId 必须是非空唯一值，因为 App 只回传 rpcId。
	const holdApproval = (req, sessionId) => {
		...
		return new Promise((resolve) => {
			const entry = { phoneId: phoneKey, sessionId, rpcId: phoneKey, remote: false, timer: null, settling: false, settle: (outcome) => { resolve(outcome); return true; } };
			pendingApprovals.set(phoneKey, entry);
			watchAbort(entry, req?.signal, () => {
				void finishApproval(entry, "cancelled");
			});
			armTimeout(entry, () => {
			...
			pendingFrameSet(`a:${phoneKey}`, frame);
			broadcast({ type: "mobile/frame", frame });
	...
	const holdQuestion = (req, sessionId) => {
		...
		return new Promise((resolve, reject) => {
			...
			pendingFrameSet(`q:${phoneKey}`, frame);
			broadcast({ type: "mobile/frame", frame });
	...
	// 没有手机连接时仍必须回 `next`，否则本插件这个唯一的 $events client
	// 会把 Host waterfall 永久挂住；有手机时才创建可回放的 pending 卡片。
	const onEventsWaterfall = (frame) => {
		const sessionId = sessionIdFromRc1AgentId(frame.agentId);
		const eventId = typeof frame.eventId === "string" && frame.eventId !== "" ? frame.eventId : "";
		const clientId = eventsState.clientId;
		if (!eventId) return;
		if (settledEventIds.has(eventId)) {
			// A stale duplicate is harmless; acknowledge it so a newly-created
			// Gateway client cannot retain an already-finished delivery.
			void postEventsNext(eventId, clientId).then((accepted) => {
				if (accepted) rememberSettledEvent(eventId);
			});
			return;
		}
		// Reconnecting the process-local $events stream replays all still-pending
		// events. Keep the existing phone entry instead of treating that replay as
		// a new no-phone request (which would incorrectly return `next`).
		if ([...pendingApprovals.values(), ...pendingQuestions.values()].some((entry) => entry.rpcId === eventId)) return;
		...
		if (frame.event === "approval/request") {
			const phoneKey = eventId;
			if ([...pendingApprovals.values()].some((entry) => entry.rpcId === eventId)) return;
			ctx.logger.info?.(`mobile-remote: approval/request 双端呈现（${sessionId} tool=${request?.toolName}）`);
			const entry = {
				phoneId: phoneKey, sessionId, rpcId: eventId, remote: true, clientId, timer: null, settling: false,
				settle: (outcome) => postEventsValue(eventId, outcome, clientId),
			};
			pendingApprovals.set(phoneKey, entry);
			armTimeout(entry, () => {
				...
				entry.timedOut = true;
				void finishApproval(entry, "unavailable");
			});
			const frame = {
		...
		if (frame.event === "user-questions/request") {
			const phoneKey = eventId;
			if ([...pendingQuestions.values()].some((entry) => entry.rpcId === eventId)) return;
			ctx.logger.info?.(`mobile-remote: user-questions/request 双端呈现（${sessionId}）`);
			const entry = {
				phoneId: phoneKey,
				sessionId,
				rpcId: eventId,
				...
				timer: null,
				settling: false,
				settle: (value) => postEventsValue(eventId, value, clientId),
				reject: (reason) => postEventsRejection(eventId, reason, clientId),
			};
			pendingQuestions.set(phoneKey, entry);
			armTimeout(entry, () => {
				ctx.logger.warn?.(`mobile-remote: user-questions/request 双端待办超时 → 取消（${phoneKey}）`);
				entry.timedOut = true;
				void finishQuestion(entry, questionCancelledError("ask_user_question timed out before the user answered"), { reject: true });
			});
			const frame = {
		...
		// Keep the Host waterfall live even if a future allowlisted event reaches
		// this plugin before a mobile presentation is implemented for it.
		void postEventsNext(eventId, clientId).then((accepted) => {
			if (accepted) rememberSettledEvent(eventId);
		});
	};
	...
	ctx.effect(() => {
		...
		if (approvalMode === "both") {
			...
			(async () => {
				let gateway;
				try { gateway = ctx.get("typertGateway"); } catch { gateway = void 0; }
				if (!gateway || typeof gateway.openWireStream !== "function") {
					ctx.logger.info?.(`mobile-remote: approvalMode=both 但宿主无 Remote 事件桥（openWireStream）→ 按 mobile 语义降级；${HOST_UPGRADE_HINT}`);
					return;
				}
				// 结算通路是网关未文档化的私有方法（ADR 0001）：缺失时提前告警，避免手机端
				// 弹出无法应答的卡片——eventsCapable() 会自然变 false，瀑布按 mobile 语义工作。
				if (typeof gateway.dispatchRpc !== "function") {
					ctx.logger.warn?.(`mobile-remote: 宿主 Remote 事件结算通路（dispatchRpc）不可用——双端同卡呈现关闭，问询/审批在手机在线时由手机独占应答；${HOST_UPGRADE_HINT}`);
				}
				// 内核 dsh-api-remotes 可能晚于本插件注册 $events 事件源（插件加载序不定）——
				// 就绪后的打开失败/流断开由下方 while 循环持续重连（含 gateway 重新获取）。
				let attempt = 0;
				while (!controller.signal.aborted) {
					...
					try {
						...
						if (!controller.signal.aborted) ctx.logger.warn?.("mobile-remote: $events 流结束，准备重连");
					} catch (err) {
						if (controller.signal.aborted) return;
						ctx.logger.warn?.(`mobile-remote: $events 流断开（${err?.message ?? err}），准备重连`);
						gateway = void 0;
					}
					if (controller.signal.aborted) return;
					discardRemotePending();
					eventsState.ready = false;
					eventsState.clientId = "";
					attempt = Math.min(attempt + 1, 5);
					await new Promise((resolve) => setTimeout(resolve, 1000 * Math.max(1, attempt)));
				}
			})();
		}
		...
		return () => {
			...
			for (const entry of pendingApprovals.values()) {
				if (entry.timer) clearTimeout(entry.timer);
				try { Promise.resolve(entry.settle("unavailable")).catch(() => {}); } catch {}
			}
			for (const entry of pendingQuestions.values()) {
				if (entry.timer) clearTimeout(entry.timer);
				try { Promise.resolve(entry.reject?.(questionCancelledError())).catch(() => {}); } catch {}
			}
			pendingApprovals.clear();
			pendingQuestions.clear();
			pendingFrames.clear();
			settledEventIds.clear();
		};
	});
	...
	ctx.effect(() => {
		...
		loadHeld();
		loadReceipts();
		// ── 问询/审批帧桥（旧代宿主专用；ADR 0001 弃用通道）──────────────────
		// 旧代内核（0.1.1-rc.2 及更早）的交互通道：订阅 apiProxy 的 mux 队列，只转发
		// question/approval 瞬态帧，应答经 proxy.respond 回写。当前代际已由上游移除该服务
		// （@deepseek-ai/dsh-host-apiproxy 最高只发布到 0.1.1-rc.2），新代宿主走 lib 上方的
		// Agent 作用域瀑布 + $events 通路。
		//
		// 保留原因：它不依赖网关未文档化的 dispatchRpc，是结算通路失效时的唯一降级出口。
		// 服务获取用 ctx.inject 而非 ctx.get：apiProxy 的依赖链比本插件的 webServer 更深，
		// 插件装配时它通常尚未 ACTIVE（**不是**上下文隔离——同 realm 下 ctx.get 可见兄弟服务）。
		try {
			ctx.inject(["apiProxy"], (apiCtx) => {
				...
				// 命中即说明宿主仍是旧代（新代宿主不提供 apiProxy）：一次性弃用提示。
				if (!legacyHostWarned) {
					legacyHostWarned = true;
					ctx.logger.warn?.(`mobile-remote: 检测到旧代宿主交互通道（apiProxy 帧桥）—— 旧代宿主仍可用，但已不再受支持；${HOST_UPGRADE_HINT}`);
				}
				(async () => {
					try {
						for await (const envelope of proxy.events.mux({}, sig.signal)) {
							...
							if (frame.type === "question/requested" || frame.type === "approval/requested") {
								...
								// 旧代宿主：内核直接广播 session/queue 快照。新代宿主改由
								// onSessionEvent 的 agent/inbox/spliced 承担（见该函数），
								// 故此处保留以覆盖仍在旧代的部署。
		...
		return () => {
			...
			unsubscribeGitOperations?.();
			if (ownsGitRemote) gitRemote.stop?.();
			if (ownsGitWrites) gitWrites.stop?.();
			if (ownsGit) git.stop?.();
			if (ownsGitOperations) gitOperations.stop?.();
