...
/** 校验 RC1 问询结果，避免非法选项在 Gateway 结算前污染上游状态。 */
...
export function apply(ctx, config) {
	...
	const publishRemoteWaterfall = (frame) => {
		...
		if (frame.event === "user-questions/request") {
			...
			const mobileQuestions = Array.isArray(request.questions) ? request.questions : [];
			const pending = { clientId: remoteEventClientId, eventId: frame.eventId, kind: "question", sessionId, questions: mobileQuestions };
	...
	const respondRemoteWaterfall = async (body) => {
		...
		if (body?.kind === "question") {
			...
			const validation = validateQuestionAnswers(pending.questions, body.answers);
			if (!validation.ok) throw Object.assign(new Error(validation.detail), { status: 400, code: validation.code });
