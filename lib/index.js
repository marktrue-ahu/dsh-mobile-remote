...
export function apply(ctx, config) {
	...
	if (!gitWrites) {
		ownsGitWrites = true;
		gitWrites = createGitWriteService({ git, operations: gitOperations, preconditionSecret: loadProviderSecret(), onChanged: (repositoryId, changeKinds) => broadcast({ type: "git/changed", repositoryId, changeKinds }) });
		ctx.provide("gitWriteService", gitWrites);
	}
	const gitCapabilities = () => {
		const base = git.capabilities();
		const writes = gitWrites.capabilities?.() ?? {};
		const features = { ...(base.features ?? {}), ...(writes.features ?? {}) };
