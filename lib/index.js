...
import { createGitWriteService } from "./git-write-service.js";
...
export function apply(ctx, config) {
	...
	if (!gitWrites) {
		ownsGitWrites = true;
		gitWrites = createGitWriteService({ git, operations: gitOperations, onChanged: (repositoryId, changeKinds) => broadcast({ type: "git/changed", repositoryId, changeKinds }) });
		ctx.provide("gitWriteService", gitWrites);
	}
	const gitCapabilities = () => {
		const base = git.capabilities();
		const writes = gitWrites.capabilities?.() ?? {};
		return { ...base, ...writes, features: { ...(base.features ?? {}), ...(writes.features ?? {}) } };
	};
	...
	const handleApi = async (req, res, url, rest) => {
		...
		if (method === "POST" && ["/git/change-sets", "/git/stage", "/git/unstage", "/git/commit/preflight", "/git/commit"].includes(rest)) {
			try {
				...
				if (rest === "/git/stage" || rest === "/git/unstage") {
					...
					sendJson(res, 202, { ok: true, operation: value });
				} else {
		...
		if (rest === "/git/operations" || operationDetail || operationCancel) {
			try {
				...
				if (operationCancel) {
					...
					sendJson(res, 202, { ok: true, ...value });
					return;
				}
