...
export function createGitRemoteService({ git, operations, now = () => Date.now(), onChanged, preconditionSecret, providerSecret } = {}) {
	...
	const optional = async (root, args, signal, options = {}) => {
		...
		catch (error) { if (signal?.aborted || ["cancelled", "provider-unavailable", "git-provider-unavailable", "repository-busy", "workspace-not-allowed", "not-git-repository"].includes(error?.code)) throw error; return null; }
	};
	...
	const validateBranch = async (root, branch, signal) => {
		...
		try { await command(root, ["check-ref-format", `refs/heads/${branch}`], signal); }
			...
			if (error?.code === "git-command-failed" && error?.exitCode === 1) throw invalid("branch must be a valid branch name");
			throw error;
		}
		return branch;
	};
	...
	const metadataValue = async (root, name, signal) => {
		const path = await optional(root, ["rev-parse", "--git-path", name], signal);
		if (!path) return null;
		try { return trim(readFileSync(resolve(root, path), "utf8")) || null; } catch { return null; }
	};
	const rebaseAbortTarget = async (root, signal) => {
		for (const directory of ["rebase-merge", "rebase-apply"]) {
			const location = await optional(root, ["rev-parse", "--git-path", directory], signal);
			if (!location || !existsSync(resolve(root, location))) continue;
			const originalHead = await metadataValue(root, `${directory}/orig-head`, signal) ?? await optional(root, ["rev-parse", "--verify", "ORIG_HEAD"], signal);
			const headName = await metadataValue(root, `${directory}/head-name`, signal);
			return { head: isOid(originalHead) ? originalHead : null, branch: headName?.startsWith("refs/heads/") ? headName.slice("refs/heads/".length) : null };
		}
		return { head: await optional(root, ["rev-parse", "--verify", "ORIG_HEAD"], signal), branch: null };
	};
	...
	const check = async (operation) => {
		...
		try {
			...
			if (operation.kind === "git.abort") {
				...
				if (state.kind !== record.expected.kind || (record.expected.intermediateDigest && state.intermediateDigest !== record.expected.intermediateDigest) || (record.expected.stateVersion && state.stateVersion !== record.expected.stateVersion) || (record.expected.coordinationDigest && digest(domains) !== record.expected.coordinationDigest)) throw stableError("state-changed", "Git intermediate state changed", { requiresRefresh: true });
				return;
			}
	...
	const integrateStage = async ({ operation, root, params, targetOid, stages, stageId = "integrate", update, signal }) => {
		...
		stageUpdate(stages, stageId, { status: "running", preFacts: pre }, update);
		if (!isOid(targetOid) || !(await optional(root, ["rev-parse", "--verify", `${targetOid}^{commit}`], signal))) {
		...
		let mutationStarted = false;
		try {
			const contains = pre.head && await isAncestor(root, targetOid, pre.head, signal);
			if (contains) {
			...
			mutationStarted = true;
			const args = params.strategy === "rebase" ? ["rebase", "--no-autostash", targetOid] : ["merge", "--no-edit", targetOid];
			await command(root, withoutHooks(args), signal, { env: { GIT_EDITOR: "true", GIT_MERGE_AUTOEDIT: "no" } });
			const post = await localFacts(root, signal);
			const state = await inProgress(root, signal);
			if (state || !post.clean || !post.head || !(await isAncestor(root, targetOid, post.head, signal))) throw stableError("unknown-result", "integrated repository state could not be verified", { requiresRefresh: true });
	...
	const isAncestor = async (root, ancestor, descendant, signal) => {
		try { await command(root, ["merge-base", "--is-ancestor", ancestor, descendant], signal); return true; }
		catch (error) {
			if (error?.code === "git-command-failed" && error?.exitCode === 1) return false;
			throw error;
		}
	};
	...
	const runPullLike = async (operation, signal, update, sync = false) => {
		...
		if (missingRemoteTarget) {
			...
			integrationPostFacts = integrated.result?.postFacts ?? integrated.result?.preFacts;
			if (!sync) return { status: "succeeded", result: { stages, fetchResultId: fetched.result.fetchResultId, fetchedTargetOid: targetOid, headOid: integrationPostFacts?.head } };
		}
		let currentLocal;
		let destination;
		let sourceOid;
		...
		if (integrationPostFacts && !sameLocal(currentLocal, integrationPostFacts)) {
			const error = stableError("state-changed", "local repository changed after integration", { requiresRefresh: true });
	...
	const abortState = async (root, signal) => {
		...
		const abortTarget = state.kind === "rebase" ? await rebaseAbortTarget(root, signal) : { head: facts.head, branch: facts.branch };
		return { ...state, intermediateDigest: digest({ state, abortTarget }), abortTarget, head: facts.head, branch: facts.branch, stateVersion: facts.stateVersion };
	};
	...
	const abortInternal = async (operation, signal, update) => {
		...
		try {
			started = true;
			await command(root, withoutHooks([pre.kind === "rebase" ? "rebase" : "merge", "--abort"]), signal);
			const post = await inProgress(root);
			const postFacts = await localFacts(root);
			const expectedPost = pre.abortTarget ?? { head: pre.head, branch: pre.branch };
			if (post || !postFacts.clean || (expectedPost.head && postFacts.head !== expectedPost.head) || (expectedPost.branch !== null && expectedPost.branch !== undefined && postFacts.branch !== expectedPost.branch)) throw stableError("abort-incomplete", "abort did not restore the expected clean local state", { requiresRefresh: true });
			...
			const postState = await abortState(root).catch(() => null);
			const post = postState ? { kind: postState.kind, ...(postState.unmerged ? { unmerged: true } : {}) } : null;
			const postFacts = await localFacts(root).catch(() => null);
			let finalError = remoteError(error);
			const unchanged = Boolean(postState && postState.intermediateDigest === pre.intermediateDigest && postFacts?.stateVersion === pre.stateVersion);
	...
	const submitAbort = async ({ repositoryId, requestId, actor = "shared-auth-token", params = {}, preconditionToken, challengeId } = {}) => {
		...
		return operations.submit({ repositoryId, requestId, actor, kind: "git.abort", params, preconditionToken, coordinationDomains: await operationDomains(repositoryId, "git.abort", params), recoveryExempt: true, confirmation: { challengeId, requestId, coordinationDigest: challenge.coordinationDigest, expiresAt: challenge.expiresAt } });
	};
