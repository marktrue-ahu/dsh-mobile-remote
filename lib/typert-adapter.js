/**
 * DSH 0.1.2-rc.1 Typert Remote adapter.
 *
 * Keep the RC1-era (typertGateway / $events) endpoint and named-argument
 * contract in one dependency-free module so the mobile HTTP API does not
 * mirror Typert details everywhere. The module is named after the mechanism
 * (Typert Remote), not the version: when a newer DSH adjusts the contract,
 * extend this file in place instead of renaming the module per version.
 */
import { relative, isAbsolute, sep } from "node:path";

export const TYPERT_REMOTE_EVENT_RESULT = "$events/result";
...
export function typertRemoteSpec(method, payload = {}) {
...
export function validateQuestionAnswers(questions, answers) {
	...
	for (const question of questions) {
		...
		// RC1 requires `selected` on every answer, even when the user supplies
		// custom text.  Do not coerce missing/null fields: the downstream tool
		// spreads selected and would otherwise throw after this validation pass.
		...
		if (Object.hasOwn(answer, "custom")) {
			if (typeof answer.custom !== "string") return { ok: false, code: "question-answer-invalid", detail: `问题 ${question.id} 的 custom 无效` };
