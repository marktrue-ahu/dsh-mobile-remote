...
/**
 * Read one Session event snapshot across the legacy and RC1 Session surfaces.
 * RC1 keeps the mutable log private and exposes snapshotEvents(); legacy DSH
 * versions expose an events array directly.
 */
export function sessionEventsOf(session) {
	...
	if (typeof session.snapshotEvents === "function") {
		try {
			...
			// A Session can be disposed between lookup and snapshot; fall through to
			// the legacy surface instead of turning a read-only route into 500.
		}
	}
...
/**
 * Select the delivery seam for one mobile send request.
 * A cold RC1 Session is not present in agents, but session.prompt is explicitly
 * responsible for resolving/resuming it before admitting the message. The Host
 * service and Gateway flags are independent because some RC1 profiles expose
 * only one.
 */
export function rc1SendRoute({ gatewayAvailable, sessionControllerAvailable = false, sessionId, liveAgent }) {
	if (liveAgent !== undefined && liveAgent !== null) return { kind: "live", agent: liveAgent };
	if ((gatewayAvailable || sessionControllerAvailable) && typeof sessionId === "string" && sessionId !== "") {
