...
function emptyState() {
	return { generation: 0, txIds: [], operations: {}, requestIndex: {}, recoveryBlocks: {}, confirmations: {}, confirmationRequests: {} };
}
...
function normalizeState(value) {
	...
	return {
		...
		confirmations: state.confirmations && typeof state.confirmations === "object" ? state.confirmations : {},
		confirmationRequests: state.confirmationRequests && typeof state.confirmationRequests === "object" ? state.confirmationRequests : {},
	};
}
...
function applyEvent(state, event) {
	...
	if (type === "operation.confirmation-consumed") {
		if (typeof event.challengeId === "string" && event.challengeId !== "") state.confirmations[event.challengeId] = { requestId: event.requestId, operationId, at: event.at, ...(Number.isFinite(event.expiresAt) ? { expiresAt: event.expiresAt } : {}) };
		return;
	}
	if (type === "confirmations.pruned") {
		for (const key of event.requestKeys ?? []) delete state.confirmationRequests[key];
		for (const challengeId of event.challengeIds ?? []) delete state.confirmations[challengeId];
		return;
	}
	if (type === "confirmation.issued") {
		if (typeof event.key === "string" && event.key !== "") state.confirmationRequests[event.key] = { challengeId: event.challengeId, requestDigest: event.requestDigest, expiresAt: event.expiresAt, summary: event.summary };
		return;
	}
...
function confirmationExpiry(challengeId, value) {
	if (Number.isFinite(value?.expiresAt)) return value.expiresAt;
	if (typeof challengeId !== "string" || !challengeId.startsWith("challenge_") || challengeId.length > 8192) return null;
	try {
		const encoded = challengeId.slice("challenge_".length).split(".", 1)[0];
		const payload = JSON.parse(Buffer.from(encoded, "base64url").toString("utf8"));
		return Number.isFinite(payload?.expiresAt) ? payload.expiresAt : null;
	} catch { return null; }
}
...
export function createGitOperationManager({
	...
	const operationView = (operation) => {
		...
		return {
			...
			...(operation.stages ? { stages: clone(operation.stages) } : {}),
			...(operation.nextActions ? { nextActions: clone(operation.nextActions) } : {}),
			...(operation.handoff ? { handoff: clone(operation.handoff) } : {}),
			...(operation.confirmation ? { confirmation: clone(operation.confirmation) } : {}),
	...
	const updateProgress = (operationId, patch) => {
		...
		const allowed = new Set(["phase", "phaseIndex", "phaseCount", "progress", "facts", "stages", "nextActions"]);
	...
	const finishStoppedOwnership = () => {
		if (stopping && activeLeases.size === 0 && lockOwned) {
			releaseExclusive(instanceLockPath, instanceId);
			lockOwned = false;
		}
	};
	...
	const runOne = async (operationId) => {
		...
		if (domains.some(isBlocked) && operation.recoveryExempt !== true) return;
		...
		try {
			...
			for (const checker of preconditionCheckers) await checker(clone(operation));
			if (stopping) return;
		...
		try {
			do {
				...
				for (const queue of queues.values()) {
					...
					const domains = domainsOf(operation);
					if (domains.length > 0 && domains.every((domain) => queueFor(domain)[0] === operationId) && (operation.recoveryExempt === true || !domains.some(isBlocked)) && !domains.some((domain) => busyDomains.has(domain)) && executors.has(operation.kind)) {
						selected = { operationId, domains };
						break;
					}
				}
				// Recovery actions must be able to cross ordinary followers that are
				// themselves held by the recovery block. Preserve FIFO among recovery
				// actions, but do not let an ineligible ordinary queue head deadlock abort.
				if (!selected) {
					const seen = new Set();
					for (const queue of queues.values()) {
						for (const operationId of queue) {
							...
							const operation = getOperation(operationId);
							if (!operation || operation.status !== "queued" || operation.recoveryExempt !== true || !executors.has(operation.kind)) continue;
							const domains = domainsOf(operation);
							if (domains.length === 0 || domains.some((domain) => busyDomains.has(domain))) continue;
							const eligible = domains.every((domain) => {
								const domainQueue = queueFor(domain);
								const index = domainQueue.indexOf(operationId);
								return index >= 0 && !domainQueue.slice(0, index).some((id) => getOperation(id)?.recoveryExempt === true);
							});
							if (eligible) { selected = { operationId, domains }; break; }
						}
						if (selected) break;
					}
				}
				...
				for (const domain of selected.domains) {
					...
					const index = queue.indexOf(selected.operationId);
	...
	const preconditionCheckers = new Set();
	const pruneExpiredConfirmations = () => {
		const timestamp = now();
		const requestKeys = Object.entries(ledger.state.confirmationRequests).filter(([, value]) => Number.isFinite(value?.expiresAt) && value.expiresAt <= timestamp).map(([key]) => key);
		const challengeIds = Object.entries(ledger.state.confirmations).filter(([key, value]) => {
			const expiresAt = confirmationExpiry(key, value);
			return Number.isFinite(expiresAt) && expiresAt <= timestamp;
		}).map(([key]) => key);
		if (requestKeys.length > 0 || challengeIds.length > 0) commit([{ type: "confirmations.pruned", requestKeys, challengeIds }], "confirmation-pruned");
	};
	...
	const start = () => {
		...
		if (activeLeases.size > 0 || controllers.size > 0) return Promise.reject(new GitOperationError("provider-ambiguous", "previous Git operation is still stopping", { retryable: true }));
		startPromise = (async () => {
			try {
	...
	const getConfirmation = (key) => {
		checkAvailable();
		pruneExpiredConfirmations();
		return clone(ledger.state.confirmationRequests[key]);
	};
	const recordConfirmation = ({ key, challengeId, requestDigest, expiresAt, summary } = {}) => {
		checkAvailable();
		pruneExpiredConfirmations();
		if (typeof key !== "string" || key === "" || typeof challengeId !== "string" || challengeId === "" || typeof requestDigest !== "string" || requestDigest === "" || !Number.isFinite(expiresAt)) throw new GitOperationError("invalid-argument", "invalid confirmation record");
		const existing = ledger.state.confirmationRequests[key];
		if (existing && existing.expiresAt > now()) {
			if (existing.requestDigest !== requestDigest) throw new GitOperationError("idempotency-conflict", "confirmation request is already bound to different parameters");
			return { ...clone(existing), deduplicated: true };
		}
		const value = { key, challengeId, requestDigest, expiresAt, summary: typeof summary === "string" ? summary.slice(0, 240) : "" };
		commit([{ type: "confirmation.issued", ...value }], "confirmation-issued");
		return { ...clone(value), deduplicated: false };
	};
	const submit = async ({ repositoryId, requestId, actor = "shared-auth-token", kind, params = {}, coordinationDomain = repositoryId, coordinationDomains, preconditionToken, stages, nextActions, recoveryExempt = false, confirmation, execute } = {}) => {
		checkAvailable();
		...
		if (confirmation !== undefined) {
			if (!confirmation || typeof confirmation !== "object" || Array.isArray(confirmation) || Object.keys(confirmation).some((key) => !["challengeId", "requestId", "coordinationDigest", "expiresAt"].includes(key)) || typeof confirmation.challengeId !== "string" || confirmation.challengeId === "" || confirmation.requestId !== requestId || (confirmation.coordinationDigest !== undefined && (typeof confirmation.coordinationDigest !== "string" || confirmation.coordinationDigest === "")) || (confirmation.expiresAt !== undefined && (!Number.isFinite(confirmation.expiresAt) || confirmation.expiresAt <= now()))) throw new GitOperationError("confirmation-required", "confirmation is invalid");
			const consumed = ledger.state.confirmations[confirmation.challengeId];
			if (consumed && consumed.requestId !== requestId) throw new GitOperationError("confirmation-required", "confirmation challenge has already been consumed");
		}
		...
		const operation = {
			...
			...(stages === undefined ? {} : { stages: clone(stages) }),
			...(nextActions === undefined ? {} : { nextActions: clone(nextActions) }),
			...(confirmation ? { confirmation: { challengeId: confirmation.challengeId, at: now(), ...(confirmation.coordinationDigest ? { coordinationDigest: confirmation.coordinationDigest } : {}), ...(Number.isFinite(confirmation.expiresAt) ? { expiresAt: confirmation.expiresAt } : {}) } } : {}),
			...(recoveryExempt ? { recoveryExempt: true } : {}),
			createdAt: now(), updatedAt: now(), recoveryBlocked: !recoveryExempt && domains.some(isBlocked),
		};
		commit([
			{ type: "operation.created", operationId, requestKey, requestDigest, operation },
			...(confirmation ? [{ type: "operation.confirmation-consumed", operationId, challengeId: confirmation.challengeId, requestId, at: now(), ...(confirmation.coordinationDigest ? { coordinationDigest: confirmation.coordinationDigest } : {}), ...(Number.isFinite(confirmation.expiresAt) ? { expiresAt: confirmation.expiresAt } : {}) }] : []),
		], "accepted");
	...
	const handoff = async ({ repositoryId, operationId, target, reason = "conflict", requestId, expectedRevision, facts } = {}) => {
		checkAvailable();
		if (!REQUEST_ID.test(String(requestId ?? ""))) throw new GitOperationError("invalid-argument", "invalid requestId");
		if (!["computer", "model"].includes(target)) throw new GitOperationError("invalid-argument", "handoff target must be computer or model");
		const operation = getOperation(operationId);
		if (!operation || operation.repositoryId !== repositoryId) throw new GitOperationError("operation-not-found", "operation not found");
		if (operation.handoff?.requestId) {
			if (operation.handoff.requestId === requestId) { const view = operationView(operation); view.deduplicated = true; return view; }
			throw new GitOperationError("idempotency-conflict", "operation already has a different handoff request");
		}
		if (!["conflicted", "unknown-result"].includes(operation.status)) throw new GitOperationError("invalid-argument", "operation is not awaiting handoff");
		if (!Number.isInteger(expectedRevision) || expectedRevision !== operation.revision) throw new GitOperationError("state-changed", "operation revision changed", { requiresRefresh: true });
		const handoffAt = now();
		updateOperation(operationId, { handoff: { target, reason: String(reason).slice(0, 80), requestId, at: handoffAt, facts: clone(facts) }, nextActions: target === "computer" ? ["refresh", "abort"] : ["refresh", "abort"] }, "handoff");
		return operationView(operation);
	};
	...
	const manager = {
		...
		stop() {
			stopping = true;
			// Abort is only a request. Keep the instance and per-domain leases until
			// every executor promise settles; otherwise a replacement provider could
			// acquire the same lease while the old adapter is still mutating Git.
		...
		setPreconditionChecker(checker) { preconditionCheckers.clear(); if (typeof checker === "function") preconditionCheckers.add(checker); },
		addPreconditionChecker(checker) { if (typeof checker !== "function") throw new TypeError("checker required"); preconditionCheckers.add(checker); return () => preconditionCheckers.delete(checker); },
		submit,
		cancel,
		handoff,
		acknowledgeRecovery,
		...
		compact() { checkAvailable(); pruneExpiredConfirmations(); ledger.compact(); },
		capabilities() {
			return {
				...
				features: { operations: true, operationCancel: true, operationRecovery: true, operationHandoff: true },
			};
		},
	};
