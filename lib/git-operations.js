...
function applyEvent(state, event) {
	...
	if (type === "operation.confirmation-consumed") {
		if (typeof event.challengeId === "string" && event.challengeId !== "") state.confirmations[event.challengeId] = { requestId: event.requestId, operationId, at: event.at };
		return;
	}
...
export function createGitOperationManager({
	...
	const submit = async ({ repositoryId, requestId, actor = "shared-auth-token", kind, params = {}, coordinationDomain = repositoryId, coordinationDomains, preconditionToken, stages, nextActions, recoveryExempt = false, confirmation, execute } = {}) => {
		...
		if (confirmation !== undefined) {
			if (!confirmation || typeof confirmation !== "object" || Array.isArray(confirmation) || Object.keys(confirmation).some((key) => !["challengeId", "requestId", "coordinationDigest"].includes(key)) || typeof confirmation.challengeId !== "string" || confirmation.challengeId === "" || confirmation.requestId !== requestId || (confirmation.coordinationDigest !== undefined && (typeof confirmation.coordinationDigest !== "string" || confirmation.coordinationDigest === ""))) throw new GitOperationError("confirmation-required", "confirmation is invalid");
		...
		const operation = {
			...
			...(confirmation ? { confirmation: { challengeId: confirmation.challengeId, at: now(), ...(confirmation.coordinationDigest ? { coordinationDigest: confirmation.coordinationDigest } : {}) } } : {}),
		...
		commit([
			...
			...(confirmation ? [{ type: "operation.confirmation-consumed", operationId, challengeId: confirmation.challengeId, requestId, at: now(), ...(confirmation.coordinationDigest ? { coordinationDigest: confirmation.coordinationDigest } : {}) }] : []),
		], "accepted");
