...
function applyEvent(state, event) {
	...
	if (type === "operation.recovery-blocked") {
		...
		state.recoveryBlocks[operation.coordinationDomain] = operationId;
		return;
	}
	if (type === "recovery.acknowledged") {
		...
		delete state.recoveryBlocks[operation.coordinationDomain];
		operation.revision = (operation.revision ?? 0) + 1;
	}
}
...
export function createGitOperationManager({
	...
	const isBlocked = (domain) => Boolean(ledger.state.recoveryBlocks[domain]);
	...
	const acquireLease = (domain, operationId) => {
		const path = leasePathFor(domain);
		acquireExclusive(path, { instanceId, operationId, pid: process.pid, at: now() }, "repository-busy");
		activeLeases.set(operationId, { domain, path });
	};
	const releaseLease = (domain, operationId) => {
		const path = activeLeases.get(operationId)?.path ?? leasePathFor(domain);
		const owner = readJson(path);
	...
	const runOne = async (operationId) => {
		...
		try {
			...
			if (leased) releaseLease(operation.coordinationDomain, operationId);
		}
	};
	const drain = async (domain) => {
		if (draining.has(domain)) return;
		draining.add(domain);
		try {
			const queue = queueFor(domain);
			while (queue.length > 0 && !isBlocked(domain)) {
				const operationId = queue.shift();
				await runOne(operationId);
				const operation = getOperation(operationId);
				if (operation?.status === "queued" && !executors.has(operation.kind)) break;
			}
		} finally { draining.delete(domain); }
	};
	const enqueue = (operationId) => {
		const operation = getOperation(operationId);
		if (!operation || operation.status !== "queued" || isBlocked(operation.coordinationDomain)) return;
		const queue = queueFor(operation.coordinationDomain);
		if (!queue.includes(operationId)) queue.push(operationId);
		void drain(operation.coordinationDomain);
	};
	...
	const submit = async ({ repositoryId, requestId, actor = "shared-auth-token", kind, params = {}, coordinationDomain = repositoryId, preconditionToken, execute } = {}) => {
		checkAvailable();
		...
		const requestDigest = digest({ repositoryId, coordinationDomain, kind, params, preconditionToken });
		...
		const operation = {
			operationId, requestId, repositoryId, coordinationDomain, kind,
			params: clone(params), preconditionToken,
			status: "queued", revision: 1, phase: "queued", phaseIndex: 0, phaseCount: 1,
			createdAt: now(), updatedAt: now(), recoveryBlocked: isBlocked(coordinationDomain),
		};
	...
	const acknowledgeRecovery = async ({ repositoryId, operationId, facts, requestId, expectedRevision } = {}) => {
		...
		if (ledger.state.recoveryBlocks[operation.coordinationDomain] !== operationId) throw new GitOperationError("invalid-argument", "recovery is already acknowledged");
		if (!Number.isInteger(expectedRevision) || expectedRevision !== operation.revision) throw new GitOperationError("state-changed", "operation revision changed", { requiresRefresh: true });
		...
		const queued = Object.values(ledger.state.operations).filter((candidate) => candidate.coordinationDomain === operation.coordinationDomain && candidate.status === "queued" && candidate.recoveryBlocked);
		commit([
			{ type: "recovery.acknowledged", operationId, requestId, at: now(), facts: clone(facts) },
			...queued.map((candidate) => ({ type: "operation.updated", operationId: candidate.operationId, patch: { recoveryBlocked: false } })),
		], "recovery-acknowledged");
		for (const candidate of Object.values(ledger.state.operations)) {
			if (candidate.coordinationDomain === operation.coordinationDomain && candidate.status === "queued") enqueue(candidate.operationId);
		}
		return operationView(operation);
	};
	const manager = {
		...
		stop() {
			...
			for (const [operationId, lease] of activeLeases) releaseLease(lease.domain, operationId);
			activeLeases.clear();
			controllers.clear();
