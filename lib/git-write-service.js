...
export function createGitWriteService({ git, operations, now = nowMs, onChanged, preconditionSecret, providerSecret } = {}) {
	...
	const domainsFor = async (repositoryId, kind, params = {}) => {
		...
		const domains = kind === "stage" || kind === "unstage" ? [worktree] : kind === "commit" ? [common, worktree] : kind === "branch-switch" ? (params.remoteRef ? [common, worktree] : [worktree]) : kind === "branch-rename" && params.current ? [common, worktree] : [common];
	...
	const validateBranchName = async (root, name, signal) => {
		...
		catch { throw invalid("invalid local branch name"); }
		return name;
	};
	...
	const check = async (operation) => {
		...
		const token = operation.kind.startsWith("git.branch-") ? branchRecord(operation.preconditionToken) : preconditions.get(operation.preconditionToken);
		...
		if (operation.kind === "git.branch-create" || operation.kind === "git.branch-rename" || operation.kind === "git.branch-switch") {
			...
			if (operation.kind === "git.branch-create") {
				...
				if (current.head !== token.expected.head || oldOid !== token.expected.oldOid || await optional(root, ["rev-parse", "--verify", token.expected.newRef]) !== null) throw stableError("state-changed", "branch rename precondition changed", { requiresRefresh: true });
			} else {
				...
				const same = observed.head === token.expected.head && observed.indexTree === token.expected.indexTree
	...
	const validateRemoteRef = async (root, value, signal) => {
		...
		try { await command(root, ["check-ref-format", value], signal); } catch { throw invalid("remoteRef must be a remote-tracking ref"); }
	...
	const verifySwitchOutcome = async (operation, expected, root, originalError) => {
		try {
			...
			if (target && (!expected || (facts.indexTree === expected.indexTree && indexStatus === expectedStatus && files))) return { status: "succeeded", result: { branch: facts.branch, oid: facts.head } };
	...
	const branchSwitchInternal = async (operation, signal, update) => {
		...
		try {
			await command(root, remoteRef ? ["switch", "--track", "-c", targetBranch, remoteRef] : ["switch", "--no-guess", targetBranch], signal); changed = true;
	...
	const submitBranch = async ({ repositoryId, requestId, actor, action, params, preconditionToken } = {}) => {
		...
		if (branchKind === "branch-rename") coordinationParams.current = params.oldName === await optional(await repositoryRoot(repositoryId), ["symbolic-ref", "--quiet", "--short", "HEAD"]);
	...
	const submitStage = async ({ repositoryId, requestId, actor, changeSetId, selections, preconditionToken, unstage = false } = {}) => {
		...
		return operations.submit({ repositoryId, requestId, actor, kind: unstage ? "git.unstage" : "git.stage", params: { changeSetId, selections: normalized }, preconditionToken: token, coordinationDomains: await domainsFor(repositoryId, unstage ? "unstage" : "stage") });
	};
	...
	return { capabilities, createChangeSet: makeChangeSet, commitPreflight, submitStage, submitCommit, branchPreflight, submitBranch, check, stop() { changeSets.clear(); preconditions.clear(); } };
}
